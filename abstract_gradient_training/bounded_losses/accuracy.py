    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if inputs.dim() == 1 or (inputs.dim() == 2 and inputs.size(-1) == 1):
            try:
                ...
                raise ValueError(
                    f"Logit shape {inputs.shape} does not match target shape {targets.shape}."
                ) from e
            preds = inputs > 0
    ...
    def bound_forward(
        ...
        *,
        ...
        if label_k_poison > 0 or label_epsilon > 0 or poison_target_idx >= 0:
            raise ValueError(
                "Bounded accuracy loss does not support poisoning attacks."
            )
        ...
        # calculate predictions
        if inputs_l.dim() == 1 or (
            inputs_l.dim() == 2 and inputs_l.size(-1) == 1
            ...
            try:
                ...
                raise ValueError(
                    f"Logit shape {inputs_l.shape} does not match target shape {targets.shape}."
                ) from e
            ...
            assert targets.max() < inputs_l.size(
                1
            ...
            worst_case_logits = (
                1 - one_hot_labels
            ) * inputs_u + one_hot_labels * inputs_l
            best_case_logits = (
                one_hot_labels * inputs_u + (1 - one_hot_labels) * inputs_l
            )
            # calculate post-softmax output
            worst_case_preds = torch.nn.functional.softmax(
                worst_case_logits, dim=1
            ).argmax(dim=1)
            best_case_preds = torch.nn.functional.softmax(
                best_case_logits, dim=1
            ).argmax(dim=1)
        else:
            raise ValueError(
        ...
        if self.reduction == "mean":
            return (worst_case_preds == targets).float().mean(), (
                best_case_preds == targets
            ).float().mean()
        if self.reduction == "sum":
            return (worst_case_preds == targets).float().sum(), (
                best_case_preds == targets
            ).float().sum()
        return (worst_case_preds == targets).reshape(target_shape), (
            best_case_preds == targets
