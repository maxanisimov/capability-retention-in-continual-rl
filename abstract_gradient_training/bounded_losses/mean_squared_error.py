    ...
    def bound_forward(
        ...
        *,
        ...
        if poison_target_idx != -1:
            raise ValueError(
                "MSE loss does not support the poison_target_idx parameter."
            )
        ...
        interval_arithmetic.validate_interval(
            inputs_l, inputs_u, msg="mse loss input bounds."
        )
        ...
        mse_l = (
            torch.zeros_like(diffs_l)
            + (diffs_l**2) * (diffs_l > 0)
            + (diffs_u**2) * (diffs_u < 0)
        )
        mse_u = torch.maximum(diffs_l**2, diffs_u**2)
        interval_arithmetic.validate_interval(
            mse_l, mse_u, msg="mse loss value bounds."
        )
