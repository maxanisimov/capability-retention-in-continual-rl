...
class BoundedCrossEntropyLoss(BoundedLoss):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            inputs.dim() == 2
        ), f"Expected shape of (batchsize, num_classes), got {inputs.shape}"
        assert (
            targets.dim() == 1
        ), f"Expected shape of (batchsize,), got {targets.shape}"
    ...
    def backward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            softmax_outputs.shape == one_hot_targets.shape
        ), f"{one_hot_targets.shape} != {softmax_outputs.shape}"
        grad = (softmax_outputs - one_hot_targets).reshape(input_shape)
        if self.reduction == "mean":
            LOGGER.warning(
                "AGT processes grads in fragments, so using 'reduction=mean' is not recommended."
            )
            LOGGER.warning(
                "Specify 'reduction=none' and normalise manually to ensure correct scaling."
            )
        ...
        *,
        ...
        if label_epsilon > 0.0 or label_k_poison != 0 or poison_target_idx >= 0:
            raise NotImplementedError(
                "Label poisoning not implemented for cross-entropy loss forward pass."
            )
        targets = targets.squeeze() if targets.dim() > 1 else targets
        ...
        assert (
            inputs_l.dim() == 2
        ), f"Expected shape of (batchsize, num_classes), got {inputs_l.shape}"
        assert (
            inputs_u.dim() == 2
        ), f"Expected shape of (batchsize, num_classes), got {inputs_u.shape}"
        assert (
            targets.dim() == 1
        ), f"Expected shape of (batchsize,), got {targets.shape}"
        if targets.max() >= inputs_l.size(1):
            raise ValueError(
                "Labels must be in the range of the output logit dimension."
            )

        # calculate bounds on the loss
        assert label_k_poison == 0
        one_hot_labels = torch.nn.functional.one_hot(
            targets, num_classes=inputs_l.size(1)
        ).to(inputs_l.device)
        ...
        loss_l = torch.nn.functional.cross_entropy(
            best_case_logits, targets, reduction=self.reduction
        )
        loss_u = torch.nn.functional.cross_entropy(
            worst_case_logits, targets, reduction=self.reduction
        )
    ...
    def bound_backward(
        ...
        *,
        ...
        softmax_out_l, softmax_out_u = interval_arithmetic.propagate_softmax(
            inputs_l, inputs_u
        )
        ...
        assert (
            softmax_out_l.shape == one_hot_labels.shape
        ), f"Expected {one_hot_labels.shape} got {softmax_out_l.shape}"
        ...
        if label_k_poison == 0:  # no label poisoning
            ...
            one_hot_labels[:, poison_target_idx] = (
                1  # set the labels corresponding to the targeted index to 1
            )
            grad_l = (softmax_out_l - one_hot_labels).reshape(inputs_shape)
            grad_u = (softmax_out_u - 0.0).reshape(inputs_shape)
        ...
        if self.reduction == "mean":
            ...
            return (
                grad_l * n_classes / grad_l.nelement(),
                grad_u * n_classes / grad_u.nelement(),
            )
...
if __name__ == "__main__":
    ...
    assert torch.allclose(
        nominal_loss_val, loss_val
    ), f"Expected {nominal_loss_val}, got {loss_val}"
    nominal_loss_grad = torch.autograd.grad(loss_val.sum(), inputs)[0]
    loss_grad = loss.backward(inputs, targets)
    assert torch.allclose(
        nominal_loss_grad, loss_grad
    ), f"Expected {nominal_loss_grad}, got {loss_grad}"
    ...
    loss_grad_l, loss_grad_u = loss.bound_backward(
        inputs, inputs, targets, label_k_poison=10
    )
    ...
    assert torch.all(
        loss_grad <= loss_grad_u + 1e-6
    ), f"Got {loss_grad} and {loss_grad_u}"
