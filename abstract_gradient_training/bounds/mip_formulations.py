...
def add_relu_bigm(
    ...
    """
    # Check input shape
    assert x.shape == lower.shape == upper.shape
    ...
    if triangle:
        ...
        z = None
        model.addConstr(y <= upper * (x - lower) / (upper - lower))
    else:
...
def add_loss_gradient(
    m: gp.Model, logits: gp.MVar, label: np.ndarray, loss: str
) -> gp.MLinExpr:
    """
