...
def noisy_test_accuracy(
    ...
    *,
    ...
    if isinstance(model, BoundedModel):
        device = (
            torch.device(model.device) if model.device != -1 else torch.device("cpu")
        )
    else:
    ...
    assert noise_type in [
        "laplace",
        "cauchy",
    ], f"Noise type must be one of ['laplace', 'cauchy'], got {noise_type}"
    noise_level += 1e-7  # can't set distributions scale to zero
    noise_level = (
        torch.tensor(noise_level) if isinstance(noise_level, float) else noise_level
    )
    ...
    if logit_n.dim() > 1:
        raise NotImplementedError(
            "Noisy accuracy is not supported for multi-class classification."
        )
...
def compute_max_certified_k(
    ...
    for k, bounded_model in bounded_model_dict.items():
        if not all(
            torch.allclose(a, b) for a, b in zip(bounded_model.param_n, param_n_check)
        ):
            check_flag = True
        # check which points are certified for the given k_private value
        certified_points = test_metrics.certified_predictions(
            bounded_model, batch, return_proportion=False
        )
        k_max = torch.maximum(
            k_max, k * certified_points
        )  # update the max k for each point

    # warn the user if the nominal parameters don't match for all k_private values
    if check_flag:
        LOGGER.warning(
            "Nominal parameters don't match for all k_private: check that you are seeding AGT correctly"
        )
...
def compute_local_epsilons(
    ...
    local_epsilons = (
        2
        * math.log(2 / delta)
        * scipy.special.lambertw(epsilon * k_star / math.log(2 / delta))
        / k_star
    )
    # convert back to tensor. note that lambertw returns a complex number, but in our case the imaginary part is zero.
    local_epsilons = torch.tensor(
        local_epsilons.real, dtype=torch.float32, device=batch.device
    )
