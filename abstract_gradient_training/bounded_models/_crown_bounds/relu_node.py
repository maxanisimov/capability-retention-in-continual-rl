...
class ReLUNode(Node):
    ...
    def __init__(
        ...
        assert relu_lb in [
            ...
            "parallel",
            "optimizable",
        ], f"Unknown relu lower bound function: {relu_lb}"
    ...
    def update_relaxation(self) -> None:
        ...
        if (
            not self.initialized and self.relu_lb == "optimizable"
        ...
        if self.relu_lb == "one":
            ...
            self.alpha_l[self.alpha_l_opt_mask] = self.alpha_l_opt[
                self.alpha_l_opt_mask
    ...
    def _backpropagate(self, backward_bounds: LinearBounds) -> None:
        ...
        backward_bounds.Lambda = Lambda * (
            self.alpha_u.unsqueeze(1) * pos_mask + self.alpha_l.unsqueeze(1) * neg_mask
        )
        ...
        backward_bounds.Omega = Omega * (
            self.alpha_l.unsqueeze(1) * pos_mask + self.alpha_u.unsqueeze(1) * neg_mask
        )
...
if __name__ == "__main__":
    ...
    lower, upper = x.concretize()
    assert (lower <= upper).all()
    assert (lower <= x_conc).all()
    assert (upper >= x_conc).all()
