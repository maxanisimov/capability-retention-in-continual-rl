...
class IntervalBounds:
    ...
    def __or__(self, other: IntervalBounds) -> IntervalBounds:
        ...
        return IntervalBounds(
            torch.minimum(self.lb, other.lb), torch.maximum(self.ub, other.ub)
        )
    ...
    def __matmul__(self, other: IntervalBounds) -> IntervalBounds:
        ...
        return IntervalBounds(
            *interval_arithmetic.propagate_matmul(
                self.lb, self.ub, other.lb, other.ub, self.interval_matmul
            )
        )
    ...
    def transpose(self, dim1: int, dim2: int) -> IntervalBounds:
        ...
        return IntervalBounds(
            self.lb.transpose(dim1, dim2), self.ub.transpose(dim1, dim2)
        )
    ...
    @staticmethod
    def zeros(
        *args, interval_matmul: Literal["rump", "exact", "nguyen"] = "rump", **kwargs
    ) -> IntervalBounds:
