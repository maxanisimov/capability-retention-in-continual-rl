...
class IntervalBoundedModel(BoundedModel):
        ...
        *,
        ...
        for module in model:
            ...
            if isinstance(module, torch.nn.Dropout):
                LOGGER.debug(
                    "Dropout layers must be used with caution, see IntervalBoundedModel for details."
                )
        if interval_matmul not in ["rump", "exact", "nguyen"]:
            raise ValueError(
                f"Unsupported method for interval matrix multiplication: {interval_matmul}"
            )
    ...
    def bound_forward(
        ...
        for module, params_l, params_u in zip(
            ...
            if isinstance(module, torch.nn.Linear):
                ...
                W_l, W_u = params_l[0], params_u[0]
                b_l = (
                    params_l[1] if len(params_l) == 2 else None
                )  # handle the case where the bias is None
    ...
    def bound_backward(
        ...
        if not self.trainable:
            raise ValueError(
                "Model is not trainable, so backward computations are not supported."
            )
        if self._inter_l is None or self._inter_u is None:
            raise ValueError(
                "Bounded forward pass with retain_intermediate=True must be called before backward pass."
            )
        interval_arithmetic.validate_interval(
            dl_dy_l, dl_dy_u, msg="input gradient bounds"
        )

        # compare gradient argument with the forward pass output
        if dl_dy_l.shape != (out_l := self._inter_l.pop()).shape:
            raise ValueError(
                f"Gradient shape does not match the forward pass output: {out_l.shape} != {dl_dy_l.shape}"
            )
        if dl_dy_u.shape != (out_u := self._inter_u.pop()).shape:
            raise ValueError(
                f"Gradient shape does not match the forward pass output: {out_u.shape} != {dl_dy_u.shape}"
            )
        ...
        for module, params_l, params_u, inter_l, inter_u in zip(
            ...
            if isinstance(module, torch.nn.Linear):
                ...
                weight_grad_transform, input_grad_transform = (
                    _get_conv_gradient_transforms(module, inter_l)
                )
                ...
                dl_dy_l, dl_dy_u = interval_arithmetic.propagate_linear_transform(
                    ...
                    dl_dy_l,
                    dl_dy_u,
                    transform=input_grad_transform,
                )
                ...
                raise ValueError(f"Unsupported module type in IBP: {type(module)}")
            interval_arithmetic.validate_interval(
                dl_dy_l, dl_dy_u, msg=f"bounds of grad at inputs to {module}"
            )
