...
def _conv_weight_gradient(
    ...
    module_output_grad: torch.Tensor,
) -> torch.Tensor:
    """
    ...
    if isinstance(module, torch.nn.Conv1d):
        ...
        raise NotImplementedError(
            "Unsupported module type for convolutional per-sample gradients."
        )
    ...
    padding = (
        (module.padding,) * nd if isinstance(module.padding, int) else module.padding
    )
    dilation = (
        (module.dilation,) * nd if isinstance(module.dilation, int) else module.dilation
    )
    ...
    module_input = module_input.view(
        1, batch_size * module.groups, module.in_channels // module.groups, *input_shape
    )
...
class DropoutWrapper(torch.nn.Dropout):
    ...
    def forward(self, input: torch.Tensor, gen_mask=False) -> torch.Tensor:
        ...
        if self.mask is None:
            raise ValueError(
                "No dropout mask found, you may be calling forward passes in the wrong order."
            )
