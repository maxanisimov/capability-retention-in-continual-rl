...
class MIPBoundedModel(IntervalBoundedModel):
    ...
    def bound_forward(
        ...
        if retain_intermediate and not self.trainable:
            raise ValueError(
                "Intermediate values should not be retained for non-trainable models."
            )
        interval_arithmetic.validate_interval(x_l, x_u, msg="model input bounds")
        ...
        if self.transform is not None:
            ...
            interval_arithmetic.validate_interval(
                x_l, x_u, msg="bounds after transform"
            )
        ...
        for i in range(batchsize):
            if i % (batchsize // 10 + 1) == 0:
                LOGGER.debug(
                    "Solved %s bounds for %d/%d instances.",
                    self.forward_bound_mode,
                    i,
                    batchsize,
                )
            xi_l = x_l[i]
        ...
        inter_l = [
            torch.stack([act[i] for act in lower_bounds], axis=0)
            for i in range(len(lower_bounds[0]))
        ]
        inter_u = [
            torch.stack([act[i] for act in upper_bounds], axis=0)
    ...
    def _bound_forward_pass_helper(
        ...
        if self.forward_bound_mode == "miqp":
            ...
            raise ValueError(
                f"Unsupported forward bound mode: {self.forward_bound_mode}"
            )

        # add the input variable
        device = x_l.device
        h = model.addMVar(
            x_l.cpu().numpy().shape, lb=x_l.cpu().numpy(), ub=x_u.cpu().numpy()
        )
        inter_l = [x_l]
        inter_u = [x_u]

        # loop over each hidden layer
        for i, (module, params_l, params_u) in enumerate(
            zip(self.modules, self._param_l, self._param_u)
        ):
            if isinstance(module, torch.nn.Linear):
                ...
                s = _mip_bounds.add_bilinear_matmul(
                    model, W, h, W_l, W_u, x_l, x_u, relax_bilinear
                )  # s = W @ h
                ...
                if self.optimize_inter and i > 0:
                    ...
                    x_l = torch.maximum(
                        x_l, torch.from_numpy(x_l_optimized).to(device).to(x_l.dtype)
                    )
                    x_u = torch.minimum(
                        x_u, torch.from_numpy(x_u_optimized).to(device).to(x_l.dtype)
                    )
                ...
                raise ValueError(f"Unsupported module type in IBP: {type(module)}")
            interval_arithmetic.validate_interval(
                x_l, x_u, msg=f"bounds at output of {module}"
            )
            inter_l.append(x_l)
            inter_u.append(x_u)
        ...
        if not self.optimize_inter:
            x_l_optimized, x_u_optimized = _mip_bounds.bound_objective(model, h)
            inter_l[-1] = torch.maximum(
                inter_l[-1], torch.from_numpy(x_l_optimized).to(device).to(x_l.dtype)
            )
            inter_u[-1] = torch.minimum(
                inter_u[-1], torch.from_numpy(x_u_optimized).to(device).to(x_l.dtype)
            )
...
if __name__ == "__main__":
    ...
    for mode in ["miqp", "milp", "qcqp", "lp"]:
        bounded_model = MIPBoundedModel(
            test_model, forward_bound_mode=mode, optimize_inter=True
        )  # type: ignore
        ...
        custom_grads = [
            g.mean(dim=0) for g in custom_grads
        ...
        out_l, out_u = bounded_model.bound_forward(
            batch, batch, retain_intermediate=True
        )
        assert torch.allclose(out, out_l), (out - out_l).abs().max()
        assert torch.allclose(out, out_u), (out - out_u).abs().max()
        custom_grads_l, custom_grads_u = bounded_model.bound_backward(
            loss_grad, loss_grad
        )
        custom_grads_l = [
            g.mean(dim=0) for g in custom_grads_l
        ]  # reduce over the batch dimension
        custom_grads_u = [
            g.mean(dim=0) for g in custom_grads_u
        ]  # reduce over the batch dimension
