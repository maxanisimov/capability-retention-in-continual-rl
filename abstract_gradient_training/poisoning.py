...
from abstract_gradient_training.gradient_accumulation import (
    PoisoningGradientAccumulator,
)
...
def poison_certified_training(
    ...
    training_iterator = training_utils.dataloader_pair_wrapper(
        dl_train, dl_clean, config.n_epochs
    )
    val_iterator = (
    ...
    # main training loop
    for n, (batch, labels, batch_clean, labels_clean) in enumerate(
        training_iterator, 1
    ):
        ...
        # process clean data
        batch_fragments = (
            torch.split(batch_clean, config.fragsize, dim=0)
            if batch_clean is not None
            else []
        )
        label_fragments = (
            torch.split(labels_clean, config.fragsize, dim=0)
        ...
        for batch_frag, label_frag in zip(batch_fragments, label_fragments):
            ...
            # clip the gradients
            frag_grads_wp_l, frag_grads_n, frag_grads_wp_u = (
            ...
            gradient_accumulator.add_clean_fragment_gradients(
                frag_grads_n, frag_grads_wp_l, frag_grads_wp_u
            )
    ...
    LOGGER.info(
        "=================== Finished Poison Certified Training ==================="
    )
