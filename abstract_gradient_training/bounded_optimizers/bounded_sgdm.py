...
class BoundedSGDM(BoundedSGD):
    ...
    def __init__(self, bounded_model: BoundedModel, **optimizer_kwargs) -> None:
        ...
        self.l2_reg, self.l2_reg_momentum = (
            0.0,
            self.l2_reg,
        )  # we'll disable the l2 reg of the superclass

    @torch.no_grad()
    def step(
        self,
        mean_grads_l: list[torch.Tensor],
        mean_grads_n: list[torch.Tensor],
        mean_grads_u: list[torch.Tensor],
    ):
        """
        ...
        if self.l2_reg_momentum > 0:
            mean_grads_l = [
                g + self.l2_reg_momentum * p for g, p in zip(mean_grads_l, self.param_u)
            ]
            mean_grads_n = [
                g + self.l2_reg_momentum * p for g, p in zip(mean_grads_n, self.param_n)
            ]
            mean_grads_u = [
                g + self.l2_reg_momentum * p for g, p in zip(mean_grads_u, self.param_l)
            ]
        ...
        if self.momentum != 0:
            ...
            if self.step_count == 0:
                ...
                self.vel_l = [
                    self.momentum * v + (1 - self.dampening) * g
                    for v, g in zip(self.vel_l, mean_grads_l)
                ]
                self.vel_n = [
                    self.momentum * v + (1 - self.dampening) * g
                    for v, g in zip(self.vel_n, mean_grads_n)
                ]
                self.vel_u = [
                    self.momentum * v + (1 - self.dampening) * g
                    for v, g in zip(self.vel_u, mean_grads_u)
                ]
            # add to the momentum
            if self.nesterov:
                mean_grads_l = [
                    g + self.momentum * v for g, v in zip(mean_grads_l, self.vel_l)
                ]
                mean_grads_n = [
                    g + self.momentum * v for g, v in zip(mean_grads_n, self.vel_n)
                ]
                mean_grads_u = [
                    g + self.momentum * v for g, v in zip(mean_grads_u, self.vel_u)
                ]
            else:
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(
        test_model.parameters(), lr=0.01, momentum=0.9, nesterov=nesterov
    )  # type: ignore
    ...
    for _ in range(n_iters):
        ...
        loss += sum(
            l2_reg * p.norm(2) ** 2 / 2 for p in test_model.parameters()
        )  # l2 reg
    ...
    for _ in range(n_iters):
        ...
        logit_l, logit_u = bounded_model.bound_forward(
            batch, batch, retain_intermediate=True
        )
        loss_grad_l, loss_grad_u = bounded_loss.bound_backward(
            logit_l, logit_u, targets
        )
