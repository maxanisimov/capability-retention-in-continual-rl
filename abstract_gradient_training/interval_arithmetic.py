...
def propagate_affine(
    ...
    if b_l is not None or b_u is not None:
        assert (
            b_u is not None
        ), "Upper bound of bias tensor must be provided if lower bound is provided"
        assert (
            b_l is not None
        ), "Lower bound of bias tensor must be provided if upper bound is provided"
...
def propagate_matmul_exact(
    ...
    if A_l.dim() == B_l.dim() + 1:  # the case of [... x m x n] @ [... x n]
        assert A_l.size(-1) == B_l.size(
            -1
        ), f"Expected [...x m x n] @ [...x n], got {A_l.shape} @ {B_l.shape}"
        assert (
            A_l.shape[:-2] == B_l.shape[:-1]
        ), f"Expected [...x m x n] @ [...x n], got {A_l.shape} @ {B_l.shape}"
        ...
        assert A_l.size(-1) == B_l.size(
            -2
        ), f"Expected [...x m x n] @ [...x n x 1], got {A_l.shape} @ {B_l.shape}"
        assert (
            A_l.shape[:-2] == B_l.shape[:-2]
        ), f"Expected [...x m x n] @ [...x n x 1], got {A_l.shape} @ {B_l.shape}"
        ...
        assert A_l.size(-1) == B_l.size(
            -2
        ), f"Expected [...x m] @ [...x m x n], got {A_l.shape} @ {B_l.shape}"
        A_l, A_u = A_l.unsqueeze(-1), A_u.unsqueeze(-1)  # shape [... x m x 1]
        E_l, E_u = propagate_elementwise(A_l, A_u, B_l, B_u)  # shape [... x m x n]
        E_l, E_u = E_l.sum(-2), E_u.sum(-2)  # shape [... x n]
    else:
        raise ValueError(
            f"Invalid shapes for matrix multiplication: {A_l.shape} * {B_l.shape}"
        )
    return E_l, E_u
...
def propagate_norm(
    x_l: torch.Tensor, x_u: torch.Tensor, p: float = 2
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    if p == 1:
        ...
        x_square_l = torch.minimum(x_l.abs(), x_u.abs()).square() * (
            (x_l > 0) | (x_u < 0)
        )
...
def propagate_conv2d(
    ...
    **kwargs,
    ...
    if b_l is not None:
        ...
        assert (
            b_l.dim() == 1
        )  # require the bias to be a vector, even though for affine layers we require it to be 2d
...
def propagate_linear_transform(
    ...
    H_r = (
        transform(torch.abs(A_mu), B_r)
        + transform(A_r, torch.abs(B_mu))
        + transform(A_r, B_r)
    )
...
def propagate_softmax(
    A_l: torch.Tensor, A_u: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    assert (
        A_l.size() == A_u.size()
    ), "Expected input logit bounds to have the same shape"
    input_shape = A_l.shape
    A_l = A_l.view(
        A_l.size(0), -1, 1
    )  # reshape to (batch, num_classes, 1), softmax is taken over the num_classes dim
    A_u = A_u.view(A_u.size(0), -1, 1)  # reshape to (batch, num_classes, 1)
    Identity = torch.eye(A_l.shape[-2]).type(A_l.dtype).unsqueeze(0).to(A_l.device)
    # calculate bounds on the post-softmax output by choosing the best and worst case logits for each class output.
    y_l = torch.diagonal(
        torch.nn.functional.softmax(Identity * A_l + (1 - Identity) * A_u, dim=-2),
        dim1=-2,
        dim2=-1,
    )
    y_u = torch.diagonal(
        torch.nn.functional.softmax(Identity * A_u + (1 - Identity) * A_l, dim=-2),
        dim1=-2,
        dim2=-1,
    )
...
def validate_interval(
    lower: torch.Tensor | list[torch.Tensor],
    upper: torch.Tensor | list[torch.Tensor],
    ...
    """
    if isinstance(lower, list) and n is not None:
        for i, (li, ui, ni) in enumerate(zip(lower, upper, n)):
    ...
    if n is None:
        diff = torch.max(lower - upper).item()
    else:
        diff = max(
            torch.max(lower - upper).item(),
            torch.max(lower - n).item(),
            torch.max(n - upper).item(),
        )
