...
def test_mse(
    ...
    return (
        mse_u.detach().cpu().item(),
        mse_n.detach().cpu().item(),
        mse_l.detach().cpu().item(),
    )
...
def test_accuracy(
    ...
    interval_arithmetic.validate_interval(
        acc_l, acc_u, acc_n, msg="final accuracy bounds"
    )
    return (
        acc_l.detach().cpu().item(),
        acc_n.detach().cpu().item(),
        acc_u.detach().cpu().item(),
    )
...
def test_cross_entropy(
    ...
    return (
        ce_u.detach().cpu().item(),
        ce_n.detach().cpu().item(),
        ce_l.detach().cpu().item(),
    )
...
def certified_predictions(
    ...
    assert (
        logit_l.dim() == 2
    ), f"Expected shape of (batchsize, num_classes), got {logit_l.shape}"
