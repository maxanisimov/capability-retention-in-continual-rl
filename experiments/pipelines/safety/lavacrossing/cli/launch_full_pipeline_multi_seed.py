"""Launch source plus downstream LavaCrossing shield safety modes across seeds."""
...
from experiments.pipelines.safety.lavacrossing.core.paths import default_outputs_root, mode_run_dir, pipeline_root
...
MODE_TO_REQUIRED_ARTIFACTS = {
    "source": (
        ...
        "shield.pt",
        "shield_info.pt",
        "shield_safety_probabilities.png",
    ),
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run source plus downstream modes for the LavaCrossing shield-safety pipeline.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional transition slip probability override. Forwarded to source "
            "and downstream jobs."
        ),
    )
...
def _build_command(args: argparse.Namespace, *, seed: int, mode: str) -> list[str]:
    ...
    if args.slip_prob is not None:
        cmd.extend(["--slip-prob", str(args.slip_prob)])
    if mode == "source":
