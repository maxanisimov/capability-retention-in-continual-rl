...
from experiments.pipelines.safety.lavacrossing.core.config import LAYOUT_NAME
...
METRIC_SPECS = {
    ...
    "source_success_rate": MetricSpec(
        key="source_success_rate",
        label="Source Success Rate",
        nested_path=("source_success_rate",),
        flat_key="source_success_rate",
    ),
    "downstream_success_rate": MetricSpec(
        key="downstream_success_rate",
        label="Downstream Success Rate",
        nested_path=("downstream_success_rate",),
        flat_key="downstream_success_rate",
    ),
    "source_failure_rate": MetricSpec(
        key="source_failure_rate",
        label="Source Failure Rate",
        nested_path=("source_failure_rate",),
        flat_key="source_failure_rate",
    ),
    "downstream_failure_rate": MetricSpec(
        key="downstream_failure_rate",
        label="Downstream Failure Rate",
        nested_path=("downstream_failure_rate",),
        flat_key="downstream_failure_rate",
    ),
}
...
DEFAULT_CSV_NAME = "aggregate_metrics_lavacrossing_shield_safety.csv"
DEFAULT_TEX_NAME = "aggregate_metrics_lavacrossing_shield_safety.tex"
...
def _extract_env_setup(summary: dict[str, Any]) -> dict[str, object]:
    run_settings = summary.get("run_settings")
    if not isinstance(run_settings, dict):
        run_settings = {}
    out: dict[str, object] = {}
    for key in ENV_SETUP_KEYS:
...
def aggregate_metrics(
    *,
    ...
    env_setup_by_method: dict[str, dict[str, object]] = {method: {} for method in selected_methods}
    found_summary = False
    ...
    for seed_dir in seed_dirs:
        ...
        for method in selected_methods:
            ...
            if not env_setup_by_method[method]:
                env_setup_by_method[method] = _extract_env_setup(summary)
    ...
    for method in selected_methods:
        ...
        rows.append(
            MethodAggregate(
                ...
                metrics=metric_aggs,
                env_setup=env_setup_by_method[method],
            ),
        )
...
def write_csv(
    ...
    *,
    ...
    fieldnames = ["layout", "method", "method_label", "num_seeds"] + ENV_SETUP_KEYS + metric_keys
    with output_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for row in rows:
            ...
            for key in ENV_SETUP_KEYS:
                payload[key] = row.env_setup.get(key, "")
    ...
    *,
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            (
                r"\caption{LavaCrossing shield safety "
                f"({_escape_latex(layout)}): aggregated metrics across seeds.}}"
            ),
            rf"\label{{tab:lavacrossing_shield_safety_{_latex_label_fragment(layout)}_metrics}}",
            r"\end{table}",
        ],
    )
    return "\n".join(lines)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate LavaCrossing shield safety run_summary metrics across seeds and "
