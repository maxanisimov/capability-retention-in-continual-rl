"""Aggregate LavaCrossing Shield Safety per-policy metrics across seeds and export CSV + LaTeX."""
...
from experiments.pipelines.safety.lavacrossing.core.paths import default_outputs_root
...
POLICY_RENAME = {
    ...
    "downstream_safe_line_search": "SafeLineSearch",
    "downstream_lagrangian": "Lagrangian",
}
...
def _build_latex_table_from_csv(csv_path: Path, *, layout: str) -> str:
    ...
    lines.extend(
        [
            r"\bottomrule",
            r"\end{tabular}",
            rf"\caption{{LavaCrossing Shield Safety ({_escape_latex(layout)}): aggregated metrics across seeds.}}",
            rf"\label{{tab:lavacrossing_shield_safety_{_escape_latex(layout)}_aggregated_metrics}}",
            r"\end{table}",
        ],
    )
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for one LavaCrossing Shield Safety layout, then export CSV and LaTeX."
        ),
    )
    parser.add_argument(
        "--pipeline",
        ...
        dest="layout",
        default=LAYOUT_NAME,
        help=f"Layout/pipeline name under outputs (default: {LAYOUT_NAME}).",
    )
    parser.add_argument(
        "--outputs-root",
        type=Path,
