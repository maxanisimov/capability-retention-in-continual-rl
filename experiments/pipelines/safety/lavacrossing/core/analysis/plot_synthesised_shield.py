"""Synthesize and plot a LavaCrossing shield without training."""
...
def save_synthesised_shield_plot(
    *,
    ...
    if slip_prob is not None:
        resolved_slip_prob = float(slip_prob)
        if not 0.0 <= resolved_slip_prob <= 1.0:
            raise ValueError(f"slip_prob must be in [0, 1], got {resolved_slip_prob}.")
        cfg = replace(
            cfg,
            slip_prob=resolved_slip_prob,
            dynamics="deterministic" if resolved_slip_prob == 0.0 else "stochastic",
        )
    ...
    shield, info = synthesise_lavacrossing_shield(
        task_map,
    ...
    stem = basename or (
        f"lavacrossing_{layout}_{task}_{resolved_shield_type}_shield_"
        f"risk_{_clean_suffix(resolved_risk_threshold)}_slip_{_clean_suffix(cfg.slip_prob)}"
    )
    ...
    try:
        fig = plot_shield_safety_probabilities(
            ...
            title=(
                ...
                f"(risk <= {resolved_risk_threshold:g}, slip_prob={cfg.slip_prob:g})"
            ),
            seed=seed,
    ...
    metadata = {
        "layout": layout,
        "env_id": cfg.env_id,
        "pipeline_key": cfg.layout,
        "dynamics": cfg.dynamics,
        "slip_prob": float(cfg.slip_prob),
        "task": task,
        "source_or_downstream_map": list(task_map),
        "action_set": {0: "left", 1: "right", 2: "down", 3: "up", 4: "stay"},
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Synthesize a LavaCrossing shield and plot it on the environment frame.",
    )
    parser.add_argument("--pipeline", "--layout", dest="layout", default=LAYOUT_NAME)
