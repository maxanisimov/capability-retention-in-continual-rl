"""Shield probability plotting helpers for LavaCrossing."""
...
def _grid_shape_from_env(env: Any) -> tuple[int, int]:
    ...
    raise ValueError("Cannot infer LavaCrossing grid shape from environment.")
    ...
    for state in range(shield_arr.shape[0]):
        ...
        for action in visible_actions:
            ...
            if not allowed and not show_disallowed_actions:
                continue
            dx_unit, dy_unit = LAVACROSSING_ACTION_DELTAS[int(action)]
