...
from experiments.pipelines.safety.lavacrossing.core.env import ACTION_DELTAS, grid_shape, state_index_to_obs
from experiments.utils.masa_tabular_envs.gridworlds import CustomLavaCrossing
...
def _next_state_index(
    ...
    candidate = next_row * ncol + next_col
    return state_index if _cell(env_map, candidate) == "W" else candidate


def lavacrossing_transition_matrix(
    env_map: list[str] | tuple[str, ...],
    *,
    slip_prob: float = 0.0,
) -> np.ndarray:
    """Return LavaCrossing dynamics as P[next_state, state, action]."""
    nrow, ncol = grid_shape(env_map)
    n_states = nrow * ncol
    env = CustomLavaCrossing(
        desc=list(env_map),
        slip_prob=slip_prob,
    )
    try:
        ...
        env.close()
    expected_shape = (n_states, n_states, 5)
    if matrix.shape != expected_shape:
        raise ValueError(f"Expected transition matrix shape {expected_shape}, got {matrix.shape}.")
    return matrix


def lavacrossing_label_fn(env_map: list[str] | tuple[str, ...], state: int) -> set[str]:
    """Label lava as unsafe for shield synthesis."""
    cell = _cell(env_map, int(state))
    if cell == "L":
        return {"unsafe", "lava"}
    if cell == "G":
        return {"goal"}
...
def lavacrossing_cost_fn(labels: set[str]) -> float:
    return 1.0 if "unsafe" in labels else 0.0


def synthesise_lavacrossing_shield(
    env_map: list[str] | tuple[str, ...],
    *,
    shield_type: ShieldType = "probabilistic",
    ...
    """Synthesise a tabular shield for a LavaCrossing map."""
    env = CustomLavaCrossing(
        desc=list(env_map),
        slip_prob=slip_prob,
    )
    try:
        shield, info = synthesise_shield(
            env,
            lambda _env: lavacrossing_transition_matrix(
                env_map,
                slip_prob=slip_prob,
            ),
            lambda state: lavacrossing_label_fn(env_map, int(state)),
    ...
    *,
    ...
    if not states:
        return {
            ...
            "actions": torch.empty((0, 5), dtype=torch.float32),
        }
...
def safe_action_mask_for_state(
    ...
    *,
    ...
    """Return a mask for actions with no immediate probability of landing in lava."""
    cell = _cell(env_map, state_index)
    if cell in {"L", "G", "W"}:
        raise ValueError(f"State {state_index} is terminal/non-traversable ({cell}).")
    matrix = lavacrossing_transition_matrix(
        env_map,
        slip_prob=slip_prob,
    )
    unsafe_states = np.array(
        [_cell(env_map, state) == "L" for state in range(matrix.shape[0])],
        dtype=bool,
    )
    lava_prob = matrix[unsafe_states, state_index, :].sum(axis=0)
    mask = (lava_prob <= 0.0).astype(np.float32)
    return mask
...
def create_rashomon_dataset(
    ...
    *,
    ...
    _thresholded_shield, info = synthesise_lavacrossing_shield(
        env_map,
...
def validate_rashomon_payload(payload: RashomonPayload) -> None:
    ...
    if actions.ndim != 2 or actions.shape[1] != 5:
        raise ValueError(f"Expected actions tensor shape [N, 5], got {tuple(actions.shape)}.")
