"""Static configuration for the LavaCrossing shield-safety pipeline."""

from __future__ import annotations

from dataclasses import dataclass

from experiments.pipelines.safety.lavacrossing.core.reference_settings import (
    LAYOUT,
    lavacrossing_shield_safety_corridor_7x7_deterministic_settings,
    settings_for_layout,
)


REFERENCE_SETTINGS = lavacrossing_shield_safety_corridor_7x7_deterministic_settings()
...
SETTINGS_SOURCE = (
    f"LavaCrossing Shield Safety {LAYOUT} "
    "(experiments/pipelines/safety/lavacrossing/settings/source/train_source_policy_settings.yaml, "
    "experiments/pipelines/safety/lavacrossing/settings/adaptation/{ppo,ewc,rashomon}.yaml, "
    "experiments/pipelines/safety/lavacrossing/settings/tasks/envs.yaml, "
    "experiments/pipelines/safety/lavacrossing/settings/shield.yaml)"
)
N_ACTIONS = 5
OBS_DIM = 3
LAYOUT_NAME = LAYOUT
...
class PipelineConfig:
    layout: str = LAYOUT
    env_id: str = str(TASKS_SETTINGS.get("env_id", "CustomLavaCrossing-v0"))
    dynamics: str = str(TASKS_SETTINGS.get("dynamics", "deterministic"))
    ...
    slip_prob: float = float(TASKS_SETTINGS.get("slip_prob", 0.0))
...
def get_pipeline_config(layout: str) -> PipelineConfig:
    ...
    return PipelineConfig(
        layout=layout,
        env_id=str(tasks.get("env_id", "CustomLavaCrossing-v0")),
        dynamics=str(tasks.get("dynamics", "deterministic")),
        ...
        slip_prob=float(tasks.get("slip_prob", 0.0)),
        n_actions=N_ACTIONS,
