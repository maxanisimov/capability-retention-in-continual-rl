"""LavaCrossing environment helpers."""
...
def grid_shape(env_map: list[str] | tuple[str, ...]) -> tuple[int, int]:
    nrow = len(env_map)
    if nrow == 0:
        raise ValueError("LavaCrossing map is empty.")
    ncol = len(env_map[0])
    if ncol == 0 or any(len(row) != ncol for row in env_map):
        raise ValueError("LavaCrossing map must be rectangular.")
    return nrow, ncol
...
class DenseShapingWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self.lava_penalty = hole_penalty
    ...
    def step(self, action: int):
        ...
        if terminated and cell == "L":
            shaped -= self.lava_penalty
