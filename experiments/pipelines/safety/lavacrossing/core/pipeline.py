"""Training and adaptation implementation for the LavaCrossing shield-safety pipeline."""
...
from experiments.pipelines.safety.lavacrossing.core.env import make_env, state_index_to_obs
from experiments.pipelines.safety.lavacrossing.core.models import build_actor_critic
from experiments.pipelines.safety.lavacrossing.core.constrained_ppo import (
    lagrangian_ppo_train,
    safe_line_search_ppo_train,
)
from experiments.pipelines.safety.lavacrossing.core.analysis.plot_shield import (
    plot_shield_safety_probabilities,
)
from experiments.pipelines.safety.lavacrossing.core.paths import (
...
def _source_ppo_config(
    ...
    *,
    ...
    return PPOConfig(
        ...
        device=device,
        early_stop_min_steps=cfg.source_early_stop_min_steps,
        ...
        early_stop_success_rate_threshold=cfg.early_stop_success_rate_threshold,
    )
...
def _downstream_ppo_config(
    ...
    *,
    ...
    return PPOConfig(
        ...
        device=device,
        early_stop_min_steps=cfg.downstream_early_stop_min_steps,
        early_stop_reward_threshold=cfg.downstream_early_stop_reward_threshold,
        early_stop_failure_rate_threshold=cfg.downstream_early_stop_failure_rate_threshold,
        early_stop_success_rate_threshold=cfg.early_stop_success_rate_threshold,
    )
...
def _safety_critical_states(
    ...
    *,
    ...
    for state_index in traversable_nonterminal_states(env_map):
        mask = safe_action_mask_for_state(env_map, state_index, slip_prob=slip_prob)
        if float(mask.sum()) < float(mask.shape[0]):
            critical_states.append(state_index)
...
def _shield_info_summary(info: object) -> dict[str, Any]:
    winning_states = np.asarray(getattr(info, "winning_states", []), dtype=np.int64)
    safe_states = np.asarray(getattr(info, "safe_states", []), dtype=np.int64)
    summary: dict[str, Any] = {
        "shield_winning_state_count": int(winning_states.size),
        "shield_safe_state_count": int(safe_states.size),
    }
...
def _load_source_shield_metadata(source_dir: Path) -> dict[str, Any]:
    summary_path = source_dir / "run_summary.yaml"
    if not summary_path.exists():
        return {}
    data = yaml.safe_load(summary_path.read_text(encoding="utf-8"))
    if not isinstance(data, dict) or not isinstance(data.get("run_settings"), dict):
        return {}
    ...
    keys = (
        ...
        "shield_trainable_allowed_actions_mean",
    )
    return {
        f"source_{key}": settings[key]
        for key in keys
        if key in settings
    }
...
def train_source(args: argparse.Namespace) -> Path:
    ...
    shield_type = str(args.shield_type or cfg.shield_type)
    shield_risk_threshold = float(
        args.shield_risk_threshold
        if args.shield_risk_threshold is not None
        else cfg.shield_risk_threshold,
    )
    shield_theta = float(args.shield_theta if args.shield_theta is not None else cfg.shield_theta)
    shield_max_vi_steps = int(
        args.shield_max_vi_steps
        if args.shield_max_vi_steps is not None
        else cfg.shield_max_vi_steps,
    )
    unsafe_cost_threshold = float(
        args.unsafe_cost_threshold
        if args.unsafe_cost_threshold is not None
        else cfg.unsafe_cost_threshold,
    )
    shield, shield_info = synthesise_lavacrossing_shield(
        cfg.source_map,
        shield_type=shield_type,  # type: ignore[arg-type]
    ...
    shield_path = run_dir / "shield.pt"
    shield_info_path = run_dir / "shield_info.pt"
    ...
    torch.save(shield_info, shield_info_path)
    ...
    run_results.update(
        {
            ...
            "safety_finetune_trajectory_preserved": bool(finetune_result["trajectory_preserved"]),
        },
    )
    run_settings = {
        "mode": "source",
        ...
        "layout": args.layout,
        "pipeline_key": args.layout,
        "seed": args.seed,
        "env_id": cfg.env_id,
        "dynamics": cfg.dynamics,
        "slip_prob": cfg.slip_prob,
        "source_map": list(cfg.source_map),
        "downstream_map": list(cfg.downstream_map),
        "action_set": {
            "0": "left",
            "1": "right",
            "2": "down",
        ...
        "shield_max_vi_steps": shield_max_vi_steps,
        "unsafe_cost_threshold": unsafe_cost_threshold,
    ...
    artifacts = {
        ...
        "shield_info_path": str(shield_info_path),
...
def adapt_downstream(args: argparse.Namespace, *, mode: str) -> Path:
    ...
    safety_dataset_modes = {
        "downstream_rashomon",
        "downstream_safe_line_search",
        "downstream_lagrangian",
    }
    if mode in safety_dataset_modes:
        source_rashomon_dataset_path = source_dir / "rashomon_dataset.pt"
        if not source_rashomon_dataset_path.exists():
            raise FileNotFoundError(f"Source Rashomon dataset not found: {source_rashomon_dataset_path}")
    ...
    source_shield_metadata: dict[str, Any] = {}
    constrained_ppo_metadata: dict[str, Any] = {}
    if mode in safety_dataset_modes:
        assert source_rashomon_dataset_path is not None
        rashomon_payload = torch.load(source_rashomon_dataset_path, map_location="cpu", weights_only=False)
        source_shield_metadata = _load_source_shield_metadata(source_dir)
    if mode == "downstream_rashomon":
        ...
        actor_bounds_l, actor_bounds_u, bounded_model, rashomon_metadata = _compute_rashomon_bounds(
            ...
            seed=args.seed,
            n_iters=int(args.rashomon_n_iters if args.rashomon_n_iters is not None else cfg.rashomon_n_iters),
            inverse_temp_start=int(
                args.inverse_temp_start if args.inverse_temp_start is not None else cfg.inverse_temp_start,
            ),
            inverse_temp_max=int(
                args.inverse_temp_max if args.inverse_temp_max is not None else cfg.inverse_temp_max,
            ),
            checkpoint=int(args.rashomon_checkpoint if args.rashomon_checkpoint is not None else cfg.rashomon_checkpoint),
        )
    ...
    if mode == "downstream_ewc":
    ...
    elif mode in {"downstream_safe_line_search", "downstream_lagrangian"}:
        assert rashomon_payload is not None
        ...
        if mode == "downstream_safe_line_search":
            actor, critic, training_data, constrained_stats = safe_line_search_ppo_train(
                train_env,
                ppo_cfg,
                source_safety_payload=rashomon_payload,
                actor_warm_start=source_actor,
                critic_warm_start=source_critic,
                inverse_temp_start=inverse_temp_start,
                inverse_temp_max=inverse_temp_max,
                max_backtracks=int(args.safe_line_search_max_backtracks),
                backtrack_coef=float(args.safe_line_search_backtrack_coef),
                early_stop_eval_env=early_stop_env,
                return_training_data=True,
            )
        else:
            actor, critic, training_data, constrained_stats = lagrangian_ppo_train(
                train_env,
                ppo_cfg,
                source_safety_payload=rashomon_payload,
                actor_warm_start=source_actor,
                ...
                inverse_temp_start=inverse_temp_start,
                inverse_temp_max=inverse_temp_max,
                lambda_init=float(args.lagrangian_lambda_init),
                lambda_lr=float(args.lagrangian_lambda_lr),
                lambda_max=float(args.lagrangian_lambda_max),
                early_stop_eval_env=early_stop_env,
                return_training_data=True,
            )
        constrained_ppo_metadata = {
            "inverse_temp_start": int(inverse_temp_start),
            "inverse_temp_max": int(inverse_temp_max),
            "source_safety_dataset_size": int(rashomon_payload["state"].shape[0]),
            **constrained_stats.as_dict(),
        }
    else:
        if mode == "downstream_rashomon":
    ...
    if mode in {"downstream_safe_line_search", "downstream_lagrangian"}:
        assert rashomon_payload is not None
        source_safety_dataset_path = run_dir / "source_safety_dataset.pt"
        torch.save(rashomon_payload, source_safety_dataset_path)
        artifacts["source_safety_dataset_path"] = str(source_safety_dataset_path)
        ...
        run_settings.update(
            {
                ...
                "rashomon_n_iters": int(args.rashomon_n_iters if args.rashomon_n_iters is not None else cfg.rashomon_n_iters),
                "inverse_temp_start": int(
                    args.inverse_temp_start if args.inverse_temp_start is not None else cfg.inverse_temp_start,
                ),
                "inverse_temp_max": int(
                ...
                "rashomon_checkpoint": int(
                    args.rashomon_checkpoint if args.rashomon_checkpoint is not None else cfg.rashomon_checkpoint,
                ),
    ...
    if mode in {"downstream_safe_line_search", "downstream_lagrangian"}:
        run_settings.update(
            {
                "constraint_source": "bound_multi_label_accuracy_margin",
                "constraint_aggregation": "min",
                "constraint_required_margin": "> 0",
                "source_safety_dataset_source": str(source_rashomon_dataset_path),
                **source_shield_metadata,
                **constrained_ppo_metadata,
            },
        )
    if mode == "downstream_safe_line_search":
        run_settings.update(
            {
                "safe_line_search_max_backtracks": int(args.safe_line_search_max_backtracks),
                "safe_line_search_backtrack_coef": float(args.safe_line_search_backtrack_coef),
            },
        )
        ...
        run_settings.update(
            {
                ...
                "lagrangian_lambda_lr": float(args.lagrangian_lambda_lr),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the LavaCrossing shield-safety pipeline.")
    parser.add_argument(
        "--mode",
        required=True,
        choices=[
    ...
    parser.add_argument("--pipeline", "--layout", type=str, dest="layout", default=LAYOUT_NAME)
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional transition slip probability override. Prefer separate pipeline keys "
            "for persisted deterministic/stochastic experiments."
        ),
    )
    ...
    parser.add_argument("--safe-line-search-max-backtracks", type=int, default=10)
    parser.add_argument("--safe-line-search-backtrack-coef", type=float, default=0.5)
    parser.add_argument("--lagrangian-lambda-init", type=float, default=1.0)
    parser.add_argument("--lagrangian-lambda-lr", type=float, default=0.05)
    parser.add_argument("--lagrangian-lambda-max", type=float, default=100.0)
...
def _config_from_args(args: argparse.Namespace) -> PipelineConfig:
    ...
    if getattr(args, "slip_prob", None) is None:
        return cfg
    slip_prob = float(args.slip_prob)
    if not 0.0 <= slip_prob <= 1.0:
        raise ValueError(f"--slip-prob must be in [0, 1], got {slip_prob}.")
    dynamics = "deterministic" if slip_prob == 0.0 else "stochastic"
    return replace(cfg, slip_prob=slip_prob, dynamics=dynamics)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        print(
            ...
            f"shield_theta={args.shield_theta if args.shield_theta is not None else cfg.shield_theta} "
            f"shield_max_vi_steps="
            f"{args.shield_max_vi_steps if args.shield_max_vi_steps is not None else cfg.shield_max_vi_steps} "
            f"unsafe_cost_threshold="
            f"{args.unsafe_cost_threshold if args.unsafe_cost_threshold is not None else cfg.unsafe_cost_threshold}",
        )
        return 0
    if args.mode == "source":
        train_source(args)
