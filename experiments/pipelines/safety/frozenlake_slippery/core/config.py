...
from experiments.pipelines.safety.frozenlake_slippery.core.reference_settings import (
    LAYOUT,
...
SETTINGS_SOURCE = (
    ...
    "(experiments/pipelines/safety/frozenlake_slippery/settings/source/train_source_policy_settings.yaml, "
    "experiments/pipelines/safety/frozenlake_slippery/settings/adaptation/{ppo,ewc,rashomon}.yaml, "
    "experiments/pipelines/safety/frozenlake_slippery/settings/tasks/envs.yaml, "
    "experiments/pipelines/safety/frozenlake_slippery/settings/shield.yaml)"
)
N_ACTIONS = 4
OBS_DIM = 3
LAYOUT_NAME = LAYOUT
...
def _failure_rate_threshold(
    ppo_settings: dict[str, object],
    eval_settings: dict[str, object] | None = None,
) -> float | None:
    explicit = _optional_float(ppo_settings, "early_stop_failure_rate_threshold")
    if explicit is not None:
        return explicit
    return None if eval_settings is None else _optional_float(eval_settings, "failure_rate")
...
class PipelineConfig:
    ...
    downstream_early_stop_failure_rate_threshold: float | None = _failure_rate_threshold(
        ADAPTATION_PPO_SETTINGS,
        REFERENCE_SETTINGS["adaptation_ppo"].get("downstream_eval"),
    )
...
def get_pipeline_config(layout: str) -> PipelineConfig:
    ...
    return PipelineConfig(
        ...
        downstream_early_stop_failure_rate_threshold=_failure_rate_threshold(
            adapt_ppo,
            s["adaptation_ppo"].get("downstream_eval"),
        ),
