"""Verified-margin constrained PPO baselines for FrozenLake slippery shield safety."""
...
class VerifiedMarginConstraint:
    ...
    def hard_accuracy(self, actor: torch.nn.Module) -> float:
        return allowed_action_accuracy(
            actor,
            {"state": self.states.detach().cpu(), "actions": self.action_masks.detach().cpu()},
            device=self.device,
        )
            ...
            payload.update(
                {
                    ...
                    "safe_line_search_alpha_mean": float(sum(self.accepted_alphas) / len(self.accepted_alphas)),
...
def _load_interpolated_params(
    ...
    *,
    ...
    with torch.no_grad():
        for param, old, candidate in zip(module.parameters(), old_values, candidate_values):
...
) -> tuple[torch.nn.Sequential, torch.nn.Sequential, dict[str, np.ndarray] | None, ConstrainedPPOStats]:
...
def _constrained_ppo_train(
    ...
    *,
    ...
    if not isinstance(env.action_space, gym.spaces.Discrete):
        raise TypeError("Verified-margin constrained PPO currently supports discrete action spaces only.")
    if not isinstance(env.observation_space, gym.spaces.Box):
        raise TypeError("Verified-margin constrained PPO expects a Box observation space.")
    ...
    if log_std is not None:
        raise RuntimeError("Unexpected continuous-action log_std for discrete constrained PPO.")
    ...
    while global_step < cfg.total_timesteps and not stop_training_early:
        ...
        if method == "safe_line_search":
            ...
            lagrangian_lambda = min(float(lambda_max), max(0.0, lagrangian_lambda + float(lambda_lr) * violation))
        ...
        if global_step % (10 * cfg.rollout_steps) < cfg.rollout_steps:
            ...
            log_msg += f" | failure_rate={failure_rate:.2f} | success_rate={success_rate:.2f}"
            log_msg += f" | margin={constraint.margin(actor):.6f} | T={constraint.temperature}"
            if method == "safe_line_search":
    ...
    if cfg.eval_episodes is not None and cfg.eval_episodes > 0:
        ...
        final_msg += f" | failure_rate={failure_rate:.2f} | success_rate={success_rate:.2f}"
        final_msg += f" | margin={constraint.margin(actor):.6f} | T={constraint.temperature}"
        print(final_msg)
