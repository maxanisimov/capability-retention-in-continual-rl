...
from experiments.pipelines.safety.frozenlake_slippery.core.env import ACTION_DELTAS, grid_shape, state_index_to_obs
...
def _gym_transition_matrix(env: gym.Env) -> np.ndarray:
    """Return Gymnasium tabular dynamics as P[next_state, state, action]."""
    ...
    for state, action_map in transitions.items():
        for action, outcomes in action_map.items():
            for probability, next_state, _reward, _terminated in outcomes:
                matrix[int(next_state), int(state), int(action)] += float(probability)
    return matrix


def frozenlake_transition_matrix(
    env_map: list[str] | tuple[str, ...],
    *,
...
def safe_action_mask_for_state(
    ...
    *,
    ...
    hole_prob = matrix[unsafe_states, state_index, :].sum(axis=0)
    mask = (hole_prob <= 0.0).astype(np.float32)
    return mask
