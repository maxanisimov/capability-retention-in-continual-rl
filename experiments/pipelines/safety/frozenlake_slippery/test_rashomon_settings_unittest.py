"""Unit tests for FrozenLake slippery shield safety diagonal_4x4 settings."""
...
def _expected_adaptation_failure_rate_threshold(reference: dict) -> float | None:
    ppo_settings = reference["adaptation_ppo"]["ppo"]
    threshold = ppo_settings["early_stop_failure_rate_threshold"]
    if threshold is not None:
        return threshold
    return reference["adaptation_ppo"]["downstream_eval"]["failure_rate"]
...
class FrozenLakeSafetyReferenceSettingsTests(unittest.TestCase):
    ...
    def test_source_ppo_matches_diagonal_4x4_settings(self) -> None:
        ...
        reference = frozenlake_slippery_shield_safety_diagonal_4x4_settings()
        ...
        self.assertEqual(ppo_cfg.early_stop_success_rate_threshold, expected["early_stop_success_rate_threshold"])
    ...
    def test_downstream_ppo_matches_diagonal_4x4_settings_for_all_adapters(self) -> None:
        cfg = get_pipeline_config("diagonal_4x4")
        reference = frozenlake_slippery_shield_safety_diagonal_4x4_settings()
        expected = reference["adaptation_ppo"]["ppo"]
        expected_failure_rate_threshold = _expected_adaptation_failure_rate_threshold(reference)
        ...
        for ppo_cfg in (
            ...
            with self.subTest(ppo_cfg=ppo_cfg):
                self.assert_ppo_matches(ppo_cfg, expected)
                self.assertIsNone(ppo_cfg.early_stop_reward_threshold)
                self.assertEqual(ppo_cfg.early_stop_failure_rate_threshold, expected_failure_rate_threshold)
                self.assertEqual(ppo_cfg.early_stop_success_rate_threshold, expected["early_stop_success_rate_threshold"])
                self.assertEqual(ppo_cfg.early_stop_min_steps, 5)
