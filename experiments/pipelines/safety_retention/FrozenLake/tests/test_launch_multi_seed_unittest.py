"""Unit tests for the FrozenLake safety multi-seed launcher."""
...
from experiments.pipelines._shared import multi_seed_launcher as shared_launcher
from experiments.pipelines.safety_retention.FrozenLake.cli import launch_multi_seed as launcher


class FrozenLakeSafetyLaunchMultiSeedTests(unittest.TestCase):
    ...
    def test_build_cmd_forwards_methods_and_run_settings(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            # build_cmd expects an already-resolved deterministic flag (main() resolves the
            # None default from the pipeline before scheduling); pass it explicitly here.
            ...
            self.assertNotIn("--no-deterministic", cmd)
