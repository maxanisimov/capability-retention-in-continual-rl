...
from experiments.pipelines.safety_retention.frozenlake.core.reference_settings import (
    LAYOUT,
...
SETTINGS_SOURCE = (
    ...
    "(experiments/pipelines/safety_retention/frozenlake/settings/source/train_source_policy_settings.yaml, "
    "experiments/pipelines/safety_retention/frozenlake/settings/adaptation/{ppo,ewc,rashomon}.yaml, "
    "experiments/pipelines/safety_retention/frozenlake/settings/tasks/{tasks,pipelines}.yaml)"
)
N_ACTIONS = 4
OBS_DIM = 3
LAYOUT_NAME = LAYOUT
...
class PipelineConfig:
    ...
    slip_probability: float = float(TASKS_SETTINGS.get("slip_probability", 0.0))
    # Stochastic-only source-policy success gate (averaged over episodes); unused when deterministic.
    source_success_eval_episodes: int = int(
        REFERENCE_SETTINGS["source"].get("source_success", {}).get("eval_episodes", 100),
    )
    source_success_rate_threshold: float = float(
        REFERENCE_SETTINGS["source"].get("source_success", {}).get("success_rate_threshold", 1.0),
    )
...
def pipeline_is_deterministic(layout: str) -> bool:
    """Return whether the pipeline's tasks use deterministic (non-slippery) dynamics."""
    return bool(settings_for_layout(layout)["tasks"].get("deterministic", True))
...
def get_pipeline_config(layout: str) -> PipelineConfig:
    ...
    return PipelineConfig(
        ...
        slip_probability=float(tasks.get("slip_probability", 0.0)),
        source_success_eval_episodes=int(
            s["source"].get("source_success", {}).get("eval_episodes", 100),
        ),
        source_success_rate_threshold=float(
            s["source"].get("source_success", {}).get("success_rate_threshold", 1.0),
        ),
