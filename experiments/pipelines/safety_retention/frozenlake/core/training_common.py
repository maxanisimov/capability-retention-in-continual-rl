...
from experiments.pipelines.safety_retention.frozenlake.core.config import PipelineConfig
from experiments.pipelines.safety_retention.frozenlake.core.env import make_env, state_index_to_obs
...
from experiments.utils.ppo_utils import PPOConfig, evaluate_with_success
...
def import_method_module(rl: str, name: str):
    ...
    try:
        return importlib.import_module(
            f"experiments.pipelines.safety_retention.frozenlake.core.methods.{rl}.{name}",
        )
...
def validate_deterministic(deterministic: bool) -> None:
    if not deterministic:
        raise NotImplementedError(
            "--no-deterministic is not supported yet: core/safety.py's safety-critical-state "
            "checks and Rashomon certificates assume deterministic FrozenLake transitions "
            "(see its module docstring). Enabling stochastic dynamics would make those safety "
            "guarantees unsound, so this is rejected until that machinery is reworked.",
        )
...
def add_common_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=True,
        help="FrozenLake transition dynamics. Only True is currently supported.",
    )
...
def ewc_ppo_config(
    ...
    *,
    ...
    from experiments.utils.ewc_ppo import EWCPPOConfig
...
def load_actor_critic(
    ...
    *,
    ...
    from experiments.pipelines.safety_retention.frozenlake.core.config import OBS_DIM
...
def load_source_for_adaptation(
    ...
    from experiments.pipelines.safety_retention.frozenlake.core.paths import resolve_source_run_dir
