...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a DQN policy for MountainCar-v0 and save final frames."
    )
    parser.add_argument("--seed", type=int, default=2)
    parser.add_argument("--max-timesteps", type=int, default=250_000)
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/mountaincar/dqn"))
    ...
    parser.add_argument("--render-seed", type=int, default=20_013)
...
def main() -> int:
    ...
    try:
        while total_timesteps < args.max_timesteps:
            ...
            print(
                f"[DQN] timesteps={total_timesteps} "
            ...
            if quick_eval.mean_return >= args.success_threshold:
                ...
                print(
                    f"[DQN] confirm_mean={confirm_eval.mean_return:.2f} "
    ...
    final_model = DQN.load(best_model_path)
    ...
    if metrics["solved"]:
        print(f"[DQN] solved; artifacts written to {args.out_dir}", flush=True)
        return 0

    print(
        f"[DQN] did not reach mean return {args.success_threshold}; "
