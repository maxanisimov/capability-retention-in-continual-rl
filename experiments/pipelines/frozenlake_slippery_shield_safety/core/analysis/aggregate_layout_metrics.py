"""Aggregate FrozenLake Slippery Shield Safety per-policy metrics across seeds and export CSV + LaTeX."""
...
from experiments.pipelines.frozenlake_slippery_shield_safety.core.paths import default_outputs_root
...
def _build_latex_table_from_csv(csv_path: Path, *, layout: str) -> str:
    ...
    lines.extend(
        [
            r"\bottomrule",
            r"\end{tabular}",
            rf"\caption{{FrozenLake Slippery Shield Safety ({_escape_latex(layout)}): aggregated metrics across seeds.}}",
            rf"\label{{tab:frozenlake_slippery_shield_safety_{_escape_latex(layout)}_aggregated_metrics}}",
            r"\end{table}",
        ],
    )
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for one FrozenLake Slippery Shield Safety layout, then export CSV and LaTeX."
        ),
    )
    parser.add_argument(
        "--pipeline",
