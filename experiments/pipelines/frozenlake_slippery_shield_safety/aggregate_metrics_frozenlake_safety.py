...
from experiments.pipelines.frozenlake_slippery_shield_safety.core.config import LAYOUT_NAME
...
DEFAULT_CSV_NAME = "aggregate_metrics_frozenlake_slippery_shield_safety.csv"
DEFAULT_TEX_NAME = "aggregate_metrics_frozenlake_slippery_shield_safety.tex"
    ...
    *,
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            (
                r"\caption{FrozenLake slippery shield safety "
                f"({_escape_latex(layout)}): aggregated metrics across seeds.}}"
            ),
            rf"\label{{tab:frozenlake_slippery_shield_safety_{_latex_label_fragment(layout)}_metrics}}",
            r"\end{table}",
        ],
    )
    return "\n".join(lines)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate FrozenLake slippery shield safety run_summary metrics across seeds and "
