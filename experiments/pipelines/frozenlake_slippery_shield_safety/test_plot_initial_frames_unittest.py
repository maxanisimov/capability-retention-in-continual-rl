"""Unit tests for FrozenLake slippery shield-safety initial-frame plotting helpers."""

from __future__ import annotations

import unittest

from experiments.pipelines.frozenlake_slippery_shield_safety.core.analysis.plot_initial_frames import (
    build_parser,
...
class FrozenLakeShieldSafetyInitialFramePlotTests(unittest.TestCase):
    def test_source_downstream_task_cfgs_use_pipeline_maps(self) -> None:
        source_cfg, downstream_cfg = source_downstream_task_cfgs("diagonal_6x6")

        self.assertEqual(source_cfg["env_map"][0], "SFHHHH")
        self.assertEqual(downstream_cfg["env_map"][0], "SFFHHH")
        self.assertEqual(source_cfg["max_episode_steps"], 36)
