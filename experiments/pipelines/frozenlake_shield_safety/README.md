# FrozenLake Shield Safety Pipeline
...
```bash
python experiments/pipelines/frozenlake_shield_safety/run_experiment.py \
  --mode source \
...
```bash
python experiments/pipelines/frozenlake_shield_safety/cli/launch_full_pipeline_multi_seed.py \
  --pipeline diagonal_4x4 \
  --seeds 0 1 2 \
  --cores 0 1 2
```
...
```bash
python experiments/pipelines/frozenlake_shield_safety/cli/launch_adaptation_multi_seed.py \
...
```bash
python experiments/pipelines/frozenlake_shield_safety/run_safe_line_search_ppo.py \
  --pipeline diagonal_4x4 \
  --seed 0
```
...
```bash
python experiments/pipelines/frozenlake_shield_safety/run_lagrangian_ppo.py \
  --pipeline diagonal_4x4 \
  --seed 0
```
...
```bash
python experiments/pipelines/frozenlake_shield_safety/aggregate_metrics_frozenlake_shield_safety.py \
  --pipeline diagonal_4x4
```
...
```bash
python experiments/pipelines/frozenlake_shield_safety/plot_initial_frames.py \
  --pipeline diagonal_10x10
```

Artifacts default to `experiments/pipelines/frozenlake_shield_safety/artifacts/runs`.
Initial-frame figures default to
`experiments/pipelines/frozenlake_shield_safety/artifacts/figures/initial_frames`.
