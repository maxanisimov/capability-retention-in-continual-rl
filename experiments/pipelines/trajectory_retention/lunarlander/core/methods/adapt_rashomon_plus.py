"""Adapt source LunarLander policy via Rashomon+ (dual-set union-PGD)."""
...
# Allow running this file directly from experiments/pipelines/trajectory_retention/lunarlander.
_REPO_ROOT = Path(__file__).resolve().parents[6]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from experiments.pipelines._shared.adaptation_utils import neutralize_task_feature
from experiments.pipelines.trajectory_retention.lunarlander.core.env.env_factory import _make_lunarlander_env
from experiments.pipelines.trajectory_retention.lunarlander.core.env.task_loading import (
    _load_task_settings,
    _resolve_lunarlander_dynamics,
)
from experiments.pipelines.trajectory_retention.lunarlander.core.methods.source_train import (
...
def create_source_rollout_rashomon_dataset(
    ...
    *,
    ...
    """Roll out NoAdapt policy and collect source-task state-action pairs."""
...
def _sample_param_list_from_bounds(
    *,
    ...
    if len(bounds_l) != len(bounds_u):
        raise ValueError(f"Bounds length mismatch: lower={len(bounds_l)} upper={len(bounds_u)}")
...
def _build_reference_actor_from_sampled_params(
    *,
    ...
    reference_actor = copy.deepcopy(source_actor_for_template).to("cpu")
    ref_params = list(reference_actor.parameters())
    if len(ref_params) != len(sampled_param_list):
        raise ValueError(
            f"Length mismatch: sampled_param_list={len(sampled_param_list)} vs actor_params={len(ref_params)}",
        )
    ...
    with torch.no_grad():
        for idx, (param, sampled_t) in enumerate(zip(ref_params, sampled_param_list)):
            if tuple(param.shape) != tuple(sampled_t.shape):
                raise ValueError(
                    f"Shape mismatch at param index {idx}: sampled={tuple(sampled_t.shape)} vs expected={tuple(param.shape)}",
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run downstream LunarLander adaptation with Rashomon+ dual-set union-PGD.",
    )
    ...
    parser.add_argument(
        "--run-subdir",
        type=str,
        default="downstream_rashomon_plus",
    ...
    parser.add_argument("--second-rashomon-n-iters", type=int, default=10_000)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Per-parameter probability of sampling the upper endpoint for reference policy creation.",
    )
    parser.add_argument(
        "--reference-bound-sample-seed",
        type=int,
        default=None,
        help="Random seed for reference-policy bound sampling. Defaults to --seed.",
    )
    ...
    if args.second_rashomon_n_iters <= 0:
        raise ValueError("--second-rashomon-n-iters must be > 0.")
    ...
    (
        first_param_bounds_l,
        first_param_bounds_u,
        first_bounded_model,
        first_selected_inverse_temp,
        first_surrogate_threshold,
        first_cert_values,
        first_selected_cert_idx,
    ...
    print(
        f"First Rashomon bounds ready: aggregation={args.rashomon_surrogate_aggregation} | "
        f"min_hard_spec={args.rashomon_min_hard_spec:.3f} | "
        f"selected_cert={first_cert_values[first_selected_cert_idx]:.4f} "
        f"(idx={first_selected_cert_idx}) | inverse_temp={first_selected_inverse_temp}",
    )
    ...
    sampled_param_list, sample_stats = _sample_param_list_from_bounds(
        bounds_l=first_param_bounds_l,
        bounds_u=first_param_bounds_u,
    ...
    reference_actor, sampled_reference_state_dict = _build_reference_actor_from_sampled_params(
        source_actor_for_template=source_actor,
        sampled_param_list=sampled_param_list,
    )

    print(
        "Sampled reference actor from first Rashomon bounds: "
        f"upper_fraction={sample_stats['upper_fraction']:.4f}",
    )

    (
        second_param_bounds_l,
        second_param_bounds_u,
        second_bounded_model,
        second_selected_inverse_temp,
        second_surrogate_threshold,
        second_cert_values,
        second_selected_cert_idx,
    ) = compute_rashomon_bounds(
        actor=copy.deepcopy(reference_actor),
        rashomon_dataset=rashomon_dataset,
        seed=args.seed,
        rashomon_n_iters=int(args.second_rashomon_n_iters),
        min_hard_spec=float(args.rashomon_min_hard_spec),
        aggregation=str(args.rashomon_surrogate_aggregation),
        inverse_temp_start=int(args.inverse_temp_start),
        inverse_temp_max=int(args.inverse_temp_max),
    ...
    print(
        f"Second Rashomon bounds ready: n_iters={args.second_rashomon_n_iters} | "
        f"selected_cert={second_cert_values[second_selected_cert_idx]:.4f} "
        f"(idx={second_selected_cert_idx}) | inverse_temp={second_selected_inverse_temp}",
    )

    union_interval_bounds_l = [first_param_bounds_l, second_param_bounds_l]
    union_interval_bounds_u = [first_param_bounds_u, second_param_bounds_u]
    ...
    print(
        f"Adapting LunarLander with Rashomon+ union-PGD | source_task={source_task_id} -> "
        f"downstream_task={downstream_task_id} | warm_critic={args.warm_start_critic} | "
        f"task_neutralization={do_task_neutralization} | intervals={len(union_interval_bounds_l)}",
    )
    ...
    first_bounded_model_path = downstream_run_dir / "rashomon_bounded_model.pt"
    first_bounds_path = downstream_run_dir / "rashomon_param_bounds.pt"

    second_bounded_model_path = downstream_run_dir / "second_rashomon_bounded_model.pt"
    second_bounds_path = downstream_run_dir / "second_rashomon_param_bounds.pt"

    union_bounds_path = downstream_run_dir / "rashomon_union_interval_param_bounds.pt"
    sampled_reference_actor_path = downstream_run_dir / "sampled_reference_actor.pt"
    ...
    torch.save(first_bounded_model, first_bounded_model_path)
    torch.save(
        {
            "param_bounds_l": first_param_bounds_l,
            "param_bounds_u": first_param_bounds_u,
        },
        first_bounds_path,
    )

    torch.save(second_bounded_model, second_bounded_model_path)
    torch.save(
        {
            "param_bounds_l": second_param_bounds_l,
            "param_bounds_u": second_param_bounds_u,
        },
        second_bounds_path,
    )
    ...
    torch.save(
        {
            "first_param_bounds_l": first_param_bounds_l,
            "first_param_bounds_u": first_param_bounds_u,
            "second_param_bounds_l": second_param_bounds_l,
            "second_param_bounds_u": second_param_bounds_u,
            "union_interval_param_bounds_l": union_interval_bounds_l,
            "union_interval_param_bounds_u": union_interval_bounds_u,
        },
        union_bounds_path,
    )
    torch.save(sampled_reference_state_dict, sampled_reference_actor_path)
    ...
    run_settings = {
        ...
        "rashomon_rollouts": int(args.rashomon_rollouts),
        "rashomon_n_iters": int(args.rashomon_n_iters),
        "second_rashomon_n_iters": int(args.second_rashomon_n_iters),
        ...
        "reference_bound_sample_seed": int(sample_seed),
        "reference_sampling": sample_stats,
    ...
    run_results = {
        ...
        "first_surrogate_threshold": float(first_surrogate_threshold),
        "first_inverse_temperature": int(first_selected_inverse_temp),
        "first_selected_certificate_index": int(first_selected_cert_idx),
        "first_selected_certificate": float(first_cert_values[first_selected_cert_idx]),
        "first_all_certificates": [float(v) for v in first_cert_values],
        "second_surrogate_threshold": float(second_surrogate_threshold),
        "second_inverse_temperature": int(second_selected_inverse_temp),
        "second_selected_certificate_index": int(second_selected_cert_idx),
        "second_selected_certificate": float(second_cert_values[second_selected_cert_idx]),
        "second_all_certificates": [float(v) for v in second_cert_values],
    ...
    artifacts = {
        ...
        "rashomon_bounded_model_path": str(first_bounded_model_path),
        "rashomon_param_bounds_path": str(first_bounds_path),
        "second_rashomon_bounded_model_path": str(second_bounded_model_path),
        "second_rashomon_param_bounds_path": str(second_bounds_path),
        "rashomon_union_interval_param_bounds_path": str(union_bounds_path),
        "sampled_reference_actor_path": str(sampled_reference_actor_path),
    ...
    print(f"Saved first Rashomon bounded model: {first_bounded_model_path}")
    print(f"Saved second Rashomon bounded model: {second_bounded_model_path}")
    print(f"Saved union Rashomon bounds: {union_bounds_path}")
    print(f"Saved sampled reference actor: {sampled_reference_actor_path}")
