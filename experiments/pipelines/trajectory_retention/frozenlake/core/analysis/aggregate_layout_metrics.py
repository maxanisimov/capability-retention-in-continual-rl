"""Aggregate FrozenLake per-policy metrics across seeds and export CSV + LaTeX."""
...
RAW_TO_AGG_METRIC = {
    ...
    "success_rate": {
        # Per-seed success rate. FrozenLake summaries may not always include this.
...
def _extract_task_metrics(
    ...
    *,
    ...
    if not isinstance(results, dict):
        # Backward compatibility for older schemas that store metrics at top level.
        results = summary
...
def _build_latex_table_from_csv(csv_path: Path, *, layout: str) -> str:
    ...
    lines.extend(
        [
            r"\bottomrule",
            r"\end{tabular}",
            rf"\caption{{FrozenLake ({_escape_latex(layout)}): aggregated metrics across seeds.}}",
            rf"\label{{tab:frozenlake_{_escape_latex(layout)}_aggregated_metrics}}",
            r"\end{table}",
        ],
    )
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for one FrozenLake layout, then export CSV and LaTeX."
        ),
    )
    parser.add_argument(
        "--pipeline",
