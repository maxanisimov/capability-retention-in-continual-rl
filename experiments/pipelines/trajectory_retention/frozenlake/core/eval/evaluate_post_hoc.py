...
from experiments.pipelines.trajectory_retention.frozenlake.core.methods.source_train import build_actor_critic, make_env_from_layout
...
POLICY_KIND_TO_DIR = {
    "source": NOADAPT_POLICY_SUBDIR,
    "downstream": "downstream_unconstrained",
...
def _policy_dir(outputs_root: Path, layout: str, seed: int, kind: str) -> Path:
    return resolve_policy_dir(outputs_root, layout, seed, kind)
...
def _infer_activation(policy_dir: Path, requested_activation: str, fallback: str = "relu") -> str:
    ...
    if summary_path.exists():
        ...
        if isinstance(summary, dict):
            ...
            if activation is None and isinstance(run_settings, dict):
                activation = run_settings.get("activation")
...
def evaluate_all_policies_across_seeds(
    *,
    ...
    scan_outputs_root = outputs_root
    if not scan_outputs_root.exists() and outputs_root != legacy_outputs_root() and legacy_outputs_root().exists():
        scan_outputs_root = legacy_outputs_root()
    ...
    if layout_filter is None:
        layout_dirs = sorted([path for path in scan_outputs_root.iterdir() if path.is_dir()], key=lambda path: path.name)
    else:
        layout_path = scan_outputs_root / layout_filter
    ...
    for layout_dir in layout_dirs:
        ...
        for seed, seed_dir in seed_dirs:
            for policy_kind in selected_policy_kinds:
                ...
                policy_dir = resolve_policy_dir(scan_outputs_root, layout, seed, policy_subdir)
...
def main() -> None:
    ...
    if output_file is None:
        output_file = seed_run_dir(args.outputs_root, args.layout, args.seed) / "post_hoc_eval.yaml"
