...
def _source_run_dir(args: argparse.Namespace, seed: int) -> Path | None:
    if args.source_run_root is None:
        return None
    return resolve_default_source_run_dir(args.source_run_root, args.layout, seed)


def _base_cmd(args: argparse.Namespace, script_name: str, seed: int) -> list[str]:
    script_path = pipeline_root() / "cli" / script_name
    cmd: list[str] = [
        sys.executable,
        str(script_path),
        "--layout",
...
def _build_job_cmd(args: argparse.Namespace, job: JobSpec) -> list[str]:
    if job.method == "unconstrained":
        cmd = _base_cmd(args, "adapt_unconstrained.py", job.seed)
    ...
    if job.method == "ewc":
        cmd = _base_cmd(args, "adapt_ewc.py", job.seed)
        cmd.extend(
            [
                "--ewc-settings-file",
    ...
    if job.method == "rashomon":
        cmd = _base_cmd(args, "adapt_rashomon.py", job.seed)
        cmd.extend(
            [
                "--rashomon-settings-file",
