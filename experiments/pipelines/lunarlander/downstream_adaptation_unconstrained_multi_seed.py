"""Run LunarLander unconstrained downstream adaptation across multiple seeds with CPU pinning."""
...
def _parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run downstream_adaptation_unconstrained.py for one task setting across multiple "
            "seeds, pinned one active run per CPU core."
        ),
    )
    parser.add_argument(
        "--task-setting",
    ...
    parser.add_argument(
        ...
        help="Path to downstream adaptation settings YAML.",
    )
    parser.add_argument(
        "--seeds",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Subdirectory name used by downstream_adaptation_unconstrained.py.",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
        ...
        help=(
            ...
            "Default: <outputs-root>/<task-setting>/multi_seed_logs/downstream_unconstrained."
        ),
    )
    parser.add_argument(
        "--poll-seconds",
...
def _build_worker_cmd(
    ...
    script_path = Path(__file__).resolve().parent / "downstream_adaptation_unconstrained.py"
    cmd: list[str] = [
        sys.executable,
        str(script_path),
        "--task-setting",
...
def main() -> int:
    ...
    log_dir = args.log_dir or (
        args.outputs_root / args.task_setting / "multi_seed_logs" / "downstream_unconstrained"
    )
    ...
    print(
        f"Launching {len(seeds)} downstream-unconstrained runs for task-setting={args.task_setting} "
    ...
    if passthrough:
        print(f"Forwarding extra args to downstream_adaptation_unconstrained.py: {' '.join(passthrough)}")
    ...
    print("\nAll downstream unconstrained seed runs completed successfully.")
    return 0
