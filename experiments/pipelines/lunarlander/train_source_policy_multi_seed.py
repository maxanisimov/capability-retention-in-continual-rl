"""Run LunarLander source-policy training across multiple seeds with CPU pinning."""
...
def _parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train LunarLander source PPO policy for multiple seeds while pinning each "
            "concurrent run to a unique CPU core."
        ),
    )
    parser.add_argument(
        "--task-setting",
    ...
    parser.add_argument(
        ...
        help="Output root passed to train_source_policy.py via --output-dir.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="cpu",
        help="Torch device passed to train_source_policy.py.",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
        ...
        help=(
            ...
            "Default: <output-dir>/<task-setting>/multi_seed_logs."
        ),
    )
    parser.add_argument(
        "--poll-seconds",
...
def _build_worker_cmd(
    ...
    train_script = Path(__file__).resolve().parent / "train_source_policy.py"
    cmd = [
        sys.executable,
        str(train_script),
        "--seed",
...
def main() -> int:
    ...
    log_dir = args.log_dir or (args.output_dir / args.task_setting / "multi_seed_logs")
    ...
    print(
        f"Launching {len(seeds)} source runs for task-setting={args.task_setting} "
    ...
    if passthrough:
        print(f"Forwarding extra args to train_source_policy.py: {' '.join(passthrough)}")
