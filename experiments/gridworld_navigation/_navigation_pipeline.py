...
# DOING: simplify the grid world and run the script
# TODO: count the number of trajectories that are safe for both Task 1 and Task 2 

### CHANGES:
# made the diagonal hazard lines shorter
#%%
# train_two_tasks.py
...
task2_actor, task2_critic = ppo_train(
    env=env2,
    cfg=task2_ppo_cfg,
    actor_warm_start=copy.deepcopy(task1_actor),  # Use SB3-style architecture
    critic_warm_start=copy.deepcopy(task1_critic)  # Use SB3-style architecture
)
...
for ep_idx, start_idx in enumerate(episode_starts[:min(num_episodes_to_plot, len(episode_starts))]):  # Limit to first 50 episodes for clarity
    ...
    traj[:, 0] = (traj[:, 0] + 1.0) / 2.0 * img_width   # X: [-0.9, 0.9] -> [0, img_width]
    traj[:, 1] = (traj[:, 1] + 1.0) / 2.0 * img_height  # Y: [-0.9, 0.9] -> [0, img_height]
    ax.plot(
        traj[:, 0], traj[:, 1], alpha=0.5, linewidth=1,
        color='tab:green'
        # color=colors[ep_idx]
    )
    ...
    # Mark end of episodes without failure
...
#%%
# Show the proportion of safe trajectories
num_safe_episodes = 0
for ep_idx, start_idx in enumerate(episode_starts):
    end_idx = episode_starts[ep_idx + 1] if ep_idx + 1 < len(episode_starts) else len(S_exp)
    if F_exp[start_idx:end_idx].sum() == 0:
        num_safe_episodes += 1
print(f"Number of safe episodes in exploration dataset: {num_safe_episodes} / {len(episode_starts)}")
...
state_trajectories[:, 0] = (state_trajectories[:, 0] + 1.0) / 2.0 * img_width
state_trajectories[:, 1] = (state_trajectories[:, 1] + 1.0) / 2.0 * img_height
...
# Update the task1 actor and critic
...
trajectory_pixels = trajectory.copy()
trajectory_pixels[:, 0] = (trajectory[:, 0] + 1) / 2 * img_width   # X: [-0.9, 0.9] -> [0, img_width]
trajectory_pixels[:, 1] = (trajectory[:, 1] + 1) / 2 * img_height  # Y: [-0.9, 0.9] -> [0, img_height]
