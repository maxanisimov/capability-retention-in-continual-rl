...
class BoundedBCEWithLogitsLoss(BoundedLoss):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        input_shape = (
            inputs.shape
        )  # save the input shape, in case we need to return the gradient
        ...
        try:
            ...
            raise RuntimeError(
                f"Logits shape {inputs.shape} does not match targets {targets.shape} for BCE."
            ) from e

        # compute the loss
        outputs = torch.sigmoid(inputs)
        bce_val = -(
            targets * torch.log(outputs + 1e-10)
            + (1 - targets) * torch.log(1 - outputs + 1e-10)
        )
    ...
    def bound_forward(
        ...
        *,
        ...
        if label_k_poison > 0 or label_epsilon > 0 or poison_target_idx >= 0:
            raise ValueError(
                "BCE loss does not support label poisoning for the forward pass."
            )
        if not ((targets == 0) + (targets == 1)).all():
            raise ValueError("Labels must be binary for binary cross-entropy loss.")
        interval_arithmetic.validate_interval(
            inputs_l, inputs_u, msg="loss input bounds"
        )
        ...
        try:
            inputs_l, inputs_u = (
                inputs_l.reshape(targets.shape),
                inputs_u.reshape(targets.shape),
            )
        except RuntimeError as e:
            raise RuntimeError(
                f"Logits shape {inputs_l.shape} does not match targets {targets.shape} for BCE."
            ) from e
        ...
        bce_val_l = -(
            targets * torch.log(best_outputs + 1e-8)
            + (1 - targets) * torch.log(1 - best_outputs + 1e-10)
        )
        bce_val_u = -(
            targets * torch.log(worst_outputs + 1e-10)
            + (1 - targets) * torch.log(1 - worst_outputs + 1e-10)
        )
    ...
    def bound_backward(
        ...
        *,
        ...
        if label_epsilon > 0:
            raise ValueError(
                "BCE loss does not support (inf-norm) perturbation to the targets."
            )
...
if __name__ == "__main__":
    ...
    nominal_loss_grad = torch.autograd.grad(loss_val.sum(), inputs, create_graph=True)[
        0
    ]
