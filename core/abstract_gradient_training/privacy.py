...
def privacy_certified_training(
    ...
    gradient_accumulator = PrivacyGradientAccumulator(
        config.k_private, config.clip_gamma, bounded_model.param_n
    )

    # returns an iterator of length n_epochs x batches_per_epoch to handle incomplete batch logic
    training_iterator = training_utils.dataloader_pair_wrapper(
        dl_train, dl_public, config.n_epochs
    )
    val_iterator = (
        training_utils.dataloader_cycle(dl_val) if dl_val is not None else None
    )

    # main training loop
    for n, (batch, labels, batch_public, labels_public) in enumerate(
        training_iterator, 1
    ):
        ...
        if val_iterator is not None:
            loss = training_utils.compute_loss(
                bounded_model, config.get_val_loss_fn(), *next(val_iterator)
            )
            LOGGER.info(
                f"Batch {n}. Loss ({config.val_loss}): {loss[0]:.3f} <= {loss[1]:.3f} <= {loss[2]:.3f}"
            )
        ...
        batch_fragments = (
            torch.split(batch_public, config.fragsize, dim=0)
            if batch_public is not None
            else []
        )
        label_fragments = (
            torch.split(labels_public, config.fragsize, dim=0)
        ...
        for batch_frag, label_frag in zip(batch_fragments, label_fragments):
            ...
            frag_grads_l, frag_grads_n, frag_grads_u = (
                training_utils.propagate_clipping(
                    frag_grads_l,
                    frag_grads_n,
                    frag_grads_u,
                    config.clip_gamma,
            ...
            gradient_accumulator.add_public_fragment_gradients(
                frag_grads_n, frag_grads_l, frag_grads_u
            )
        ...
        for batch_frag, label_frag in zip(batch_fragments, label_fragments):
            ...
            # accumulate the gradients
            gradient_accumulator.add_private_fragment_gradients(
                frag_grads_n, frag_grads_l, frag_grads_u
            )
            # the gpu memory allocations at each loop are not always collected, so we'll prompt pytorch to do so
            gc.collect()
        ...
        batchsize = (
            batch.size(0)
            if batch_public is None
            else batch.size(0) + batch_public.size(0)
        )
        update_l, update_n, update_u = gradient_accumulator.concretize_gradient_update(
            batchsize
        )
        interval_arithmetic.validate_interval(
            update_l, update_u, update_n, "final_grad_bounds"
        )
    ...
    if val_iterator is not None:
        ...
        LOGGER.info(
            f"Final Eval. Loss ({config.val_loss}): {loss[0]:.3f} <= {loss[1]:.3f} <= {loss[2]:.3f}"
        )

    training_utils.validate_bounded_model(bounded_model)
    LOGGER.info(
        "=================== Finished Privacy Certified Training ==================="
    )
