...
def get_parameters(
    ...
    for i, layer in enumerate(model):
        if i % 2 == 0:
            ...
            ), f"Expected Linear layer, got {layer}."
            params.append((layer.weight, layer.bias))
        else:
            assert isinstance(
                layer, torch.nn.ReLU
            ), f"Expected ReLU layer, got {layer}."
    params = [item for sublist in params for item in sublist]  # flatten the list
    params = [
        t if len(t.shape) == 2 else t.unsqueeze(-1) for t in params
    ]  # reshape biases to [n x 1] instead of [n]
...
def propagate_conv_layers(
    ...
    for layer in conv_layers:
        bias = layer.bias.detach() if layer.bias is not None else layer.bias
        conv_parameters.append(
            (layer.weight.detach(), bias, layer.stride, layer.padding, layer.dilation)
        )
...
def get_conv_model_transform(model: torch.nn.Sequential) -> Callable:
    ...
    for i, layer in enumerate(model):
        if i == len(model) - 1:
            ...
            ), f"Expected Flatten layer, got {layer}."
        elif i % 2 == 0:
            assert isinstance(
                layer, torch.nn.Conv2d
            ), f"Expected Conv2d layer, got {layer}."
        else:
            assert isinstance(
                layer, torch.nn.ReLU
            ), f"Expected ReLU layer, got {layer}."
