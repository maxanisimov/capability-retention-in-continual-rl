...
class BoundedSGD(BoundedOptimizer):
    ...
    def __init__(self, bounded_model: BoundedModel, **optimizer_kwargs) -> None:
        ...
        if self.l1_reg > 0 and self.l2_reg > 0:
            raise ValueError(
                "l1_reg and l2_reg cannot be used together, the bounds interact in a non-trivial way."
            )
    ...
    def step(
        ...
        for i in range(len(mean_grads_n)):
            ...
            interval_arithmetic.validate_interval(
                mean_grads_l[i], mean_grads_u[i], mean_grads_n[i], msg="pre sgd"
            )

        # apply l2 regularization
        if self.l2_reg > 0:
            mean_grads_l = [
                g + self.l2_reg * p for g, p in zip(mean_grads_l, self.param_u)
            ]
            mean_grads_n = [
                g + self.l2_reg * p for g, p in zip(mean_grads_n, self.param_n)
            ]
            mean_grads_u = [
                g + self.l2_reg * p for g, p in zip(mean_grads_u, self.param_l)
            ]
        ...
        for i in range(len(mean_grads_n)):
            ...
            interval_arithmetic.validate_interval(
                self.param_l[i], self.param_u[i], self.param_n[i], msg="post sgd"
            )
    ...
    def l1_update(self) -> None:
        ...
        for pl, pn, pu in zip(self.param_l, self.param_n, self.param_u):
            ...
            pl.data.copy_(
                torch.clamp(pl + l1_scale, max=-l1_scale).where(
                    crossing_idx, pl - l1_scale * torch.sign(pl)
                )
            )
            pu.data.copy_(
                torch.clamp(pu - l1_scale, min=l1_scale).where(
                    crossing_idx, pu - l1_scale * torch.sign(pu)
                )
            )
...
if __name__ == "__main__":
    ...
    bounded_optimizer = BoundedSGD(
        bounded_model, learning_rate=0.01, l2_reg=l2_reg, l1_reg=l1_reg
    )
