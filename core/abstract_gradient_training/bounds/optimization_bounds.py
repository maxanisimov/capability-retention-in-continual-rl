...
def bound_forward_pass(
    ...
    **kwargs,
    ...
    """
    # validate the input
    param_l, param_u, x0_l, x0_u = bound_utils.validate_forward_bound_input(
        param_l, param_u, x0_l, x0_u
    )
    device = x0_l.device
    ...
    LOGGER.debug(
        "Solved %s bounds for %d instances. Avg bound time %.2fs.",
        method,
        batchsize,
        avg_time,
    )
    ...
    activations_l = [
        np.stack([act[i] for act in lower_bounds], axis=0)
        for i in range(len(lower_bounds[0]))
    ]
    activations_u = [
        np.stack([act[i] for act in upper_bounds], axis=0)
        for i in range(len(upper_bounds[0]))
    ]
...
def _bound_forward_pass_helper(
    ...
    for i in range(0, n_layers):
        ...
        s = mip_formulations.add_bilinear_matmul(
            model, W, h, W_l, W_u, h_l, h_u, relax_bilinear
        )

        # first compute the pre-activation bounds for this layer using IBP
        h_l, h_u = bound_utils.numpy_to_torch_wrapper(
            interval_arithmetic.propagate_matmul_exact, W_l, W_u, h_l, h_u
        )
...
def bound_backward_pass(
    ...
    **kwargs,
    ...
    dL_min, dL_max, param_l, param_u, activations_l, activations_u = (
        bound_utils.validate_backward_bound_input(
            dL_min, dL_max, param_l, param_u, activations_l, activations_u
        )
    )
    ...
    LOGGER.info(
        "Optimization based bounds for backward pass not recommended, use IBP instead."
    )
    ...
    for i in range(batchsize):
        if i % (batchsize // 10 + 1) == 0:
            LOGGER.debug(
                "Solved backward pass bounds for %d/%d instances", i, batchsize
            )
    ...
    avg_time = (time.time() - start) / batchsize
    LOGGER.debug(
        "Solved backward pass bounds for %d instances. Avg bound time %.2fs.",
        batchsize,
        avg_time,
    )
    LOGGER.debug(gurobi_utils.get_gurobi_model_stats(model))
    ...
    # concatenate the results
    grads_l = [
        np.stack([g[i] for g in lower_bounds], axis=0)
        for i in range(len(lower_bounds[0]))
    ]
    grads_u = [
        np.stack([g[i] for g in upper_bounds], axis=0)
        for i in range(len(upper_bounds[0]))
    ]
...
def _bound_backward_pass_helper(
    ...
    if relax_loss:
        ...
        act = model.addMVar(
            shape=activations_l[-1].shape, lb=activations_l[-1], ub=activations_u[-1]
        )
    ...
    for i in range(len(W_l) - 1, 0, -1):
        ...
        s = mip_formulations.add_bilinear_matmul(
            model, W.T, dL, W_l[i].T, W_u[i].T, dL_min, dL_max, relax_bilinear
        )
        ...
        act = model.addMVar(
            shape=activations_l[i].shape, lb=activations_l[i], ub=activations_u[i]
        )
        # note that we define the heaviside function using the pre-activation bounds
        heaviside = mip_formulations.add_heaviside(
            model, act, activations_l[i], activations_u[i], relax_binaries
        )
        ...
        dW_min, dW_max = bound_utils.numpy_to_torch_wrapper(
            ...
            dL_min,
            dL_max,
            np.maximum(activations_l[i - 1].T, 0)
            if i - 1 > 0
            else activations_l[i - 1].T,
            np.maximum(activations_u[i - 1].T, 0)
            if i - 1 > 0
            else activations_u[i - 1].T,
            interval_matmul="exact",
        )
        # store the results
...
def bound_forward_and_backward_pass(
    ...
    *,
    ...
    avg_time = (time.time() - start) / batchsize
    LOGGER.debug(
        "Solved combined bounds for %d instances. Avg bound time %.2fs.",
        batchsize,
        avg_time,
    )
    LOGGER.debug(gurobi_utils.get_gurobi_model_stats(model))
...
def _bound_forward_and_backward_pass_helper(
    ...
    for i in range(0, n_layers):
        ...
        # add the bilinear term s = W @ h
        s = mip_formulations.add_bilinear_matmul(
            model, W_vars[-1], h, W_l, W_u, h_l, h_u, relax_bilinear
        )
        ...
        # otherwise, only optimize the bounds for intermediate activations if the flag is set.
        if (i > 0 and optimize_intermediate_bounds) or i == n_layers - 1:
            h_l_optimized, h_u_optimized = gurobi_utils.bound_objective(
                model, act_vars[-1]
            )
            if np.isinf(h_l_optimized).any() or np.isinf(h_u_optimized).any():
                LOGGER.debug(
        ...
        # add next hidden variable
        h, z = mip_formulations.add_relu_bigm(
            model, act_vars[-1], h_l, h_u, relax_binaries, False
        )
