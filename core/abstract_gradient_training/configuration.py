...
class AGTConfig(pydantic.BaseModel, extra="forbid", arbitrary_types_allowed=True):
    ...
    lr_decay: float = pydantic.Field(
        0.0, ge=0, description="Learning rate decay factor"
    )
    lr_min: float = pydantic.Field(
        0.0, ge=0, description="Minimum learning rate for learning rate scheduler"
    )
    loss: str = pydantic.Field(
        ..., pattern=f"^({'|'.join(LOSS_FNS)})$", description="Loss function"
    )
    device: str = pydantic.Field("cpu", description="Device to train the model on")
    log_level: Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"] = (
        pydantic.Field("INFO", description="Logging level")
    )
    fragsize: int = pydantic.Field(
        10000,
        gt=0,
        description="Size of fragments to split the batch into to avoid OOM",
    )
    # poisoning parameters
    k_poison: int = pydantic.Field(
        0, ge=0, description="Number of samples whose features can be poisoned"
    )
    epsilon: float = pydantic.Field(
        0, ge=0, description="Maximum perturbation for feature poisoning"
    )
    label_k_poison: int = pydantic.Field(
        0, ge=0, description="Number of samples whose labels can be poisoned"
    )
    label_epsilon: float = pydantic.Field(
        0,
        ge=0,
        description="Maximum perturbation for label poisoning (in regression settings)",
    )
    poison_target_idx: int = pydantic.Field(
        -1,
        ge=0,
        description="If specified, an attacker can only flip labels to this class.",
    )
    paired_poison: bool = pydantic.Field(
        False,
        description="Whether the label and feature poisoning are paired (i.e. the same samples are poisoned).",
    )
    # unlearning parameters
    k_unlearn: int = pydantic.Field(
        0, ge=0, description="Number of removals per batch to be certified"
    )
    # privacy and dp-sgd parameters
    k_private: int = pydantic.Field(
        0, ge=0, description="Number of removals/insertions per batch to be certified"
    )
    clip_gamma: float = pydantic.Field(
        float("inf"), gt=0, description="Gradient clipping parameter"
    )
    clip_method: Literal["norm", "clamp"] = pydantic.Field(
        "clamp", description="Method for clipping gradients"
    )
    metadata: str = pydantic.Field(
        "", description="Additional metadata to store with the configuration"
    )
    ...
    def get_bounded_optimizer(
        self, model: bounded_models.BoundedModel
    ) -> bounded_optimizers.BoundedOptimizer:
        ...
        for key in keys:
            if key in optimizer_kwargs:
                raise ValueError(
                    f"{key} in optimizer_kwargs when it is a default config value."
                )
