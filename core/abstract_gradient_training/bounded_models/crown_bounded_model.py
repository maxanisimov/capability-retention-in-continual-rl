...
class CROWNBoundedModel(IntervalBoundedModel):
    ...
    def __init__(
        ...
        *,
        ...
        if gradient_bound_mode not in ["linear", "interval"]:
            raise ValueError(
                f"Unsupported method for gradient bound computation: {gradient_bound_mode}"
            )
    ...
    def bound_forward(
        ...
        for module, params_l, params_u in zip(
            ...
            if isinstance(module, torch.nn.Linear):
                ...
                ), "Linear layer without bias is not supported"
                W_l, b_l = params_l
                W_u, b_u = params_u
                x = _crown_bounds.AffineNode(
                    x, W_l, W_u, b_l, b_u, interval_matmul=self.interval_matmul
                )
            elif isinstance(module, torch.nn.ReLU):
                x = _crown_bounds.ReLUNode(
                    x,
    ...
    def bound_backward(
        ...
        if self.gradient_bound_mode == "interval":
            return super().bound_backward(
                dl_dy_l, dl_dy_u
            )  # superclass is IntervalBoundedModel
...
if __name__ == "__main__":
    ...
    out_l, out_u = bounded_model.bound_forward(
        batch - 0.1, batch + 0.1, retain_intermediate=True
    )
