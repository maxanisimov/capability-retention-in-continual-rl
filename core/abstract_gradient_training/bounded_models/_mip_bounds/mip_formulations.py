...
def tensor_to_numpy(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        new_args = [
            arg.cpu().numpy() if isinstance(arg, torch.Tensor) else arg for arg in args
        ]
...
def add_relu_bigm(
    ...
    # Check input shape
    assert (
        x.shape == lower.shape == upper.shape
    ), f"Shapes of x, l, and u must match, got {x.shape}, {lower.shape}, {upper.shape}"

    # Define output variable
    y = model.addMVar(x.shape, lb=np.maximum(lower, 0), ub=np.maximum(upper, 0))

    # Add big-M constraints
    model.addConstr(y >= x)
    model.addConstr(y >= 0)
    model.addConstr(x <= upper)
    model.addConstr(x >= lower)
    ...
    if triangle:
        lower = np.minimum(lower, 0)
        upper = np.maximum(upper, 0)
        z = None
        model.addConstr((upper - lower) * y <= upper * (x - lower))
    else:
        z = model.addMVar(x.shape, lb=0.0, ub=1.0, vtype=vtype)
        model.addConstr(y <= x - lower * (1 - z))
        model.addConstr(y <= upper * z)
...
def add_bilinear_matmul(
    ...
    assert (
        h.shape == h_l.shape == h_u.shape == (n,)
    ), f"got shapes {h.shape}, {h_l.shape}, {h_u.shape}, expected {(n,)}"
...
@tensor_to_numpy
def add_heaviside(
    m: gp.Model, x: gp.MVar, lower: np.ndarray, upper: np.ndarray, relax_binaries: bool
) -> gp.MVar:
    """
    ...
    m.addConstr(x <= z * upper)
    m.addConstr(x >= (1 - z) * lower)
    m.addConstr(np.heaviside(lower, 0) <= z)
    m.addConstr(np.heaviside(upper, 0) >= z)
    return z
...
if __name__ == "__main__":
    ...
    lower = np.array([-1, -1])
    upper = np.array([1, 1])
    y, z = add_relu_bigm(m, x, lower, upper, relax=False)
    m.optimize()
