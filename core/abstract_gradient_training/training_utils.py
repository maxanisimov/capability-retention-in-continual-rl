...
def compute_batch_gradients(
    ...
    if nominal and poisoned:
        raise ValueError(
            "Nominal gradients cannot be computed wrt the poisoning adversary."
        )
...
def compute_loss(
    ...
    val_loss_l, val_loss_u = bounded_loss.bound_forward(
        logits_val_l, logits_val_u, labels
    )
    return val_loss_l.item(), val_loss_n.item(), val_loss_u.item()


def dataloader_wrapper(
    dl_train: Iterable, n_epochs: int
    ...
    for n in range(n_epochs):
        ...
        if n == 0 and t == 0:
            LOGGER.warning(
                "Dataloader has only one batch per epoch, effective batchsize may be smaller than expected."
            )
    ...
    for n in range(n_epochs):
        ...
        if dl_aux is None:
            data_iterator: Iterable = (
                ((b, loader), (None, None)) for b, loader in dl_train
            )
        else:
            data_iterator = zip(
                dl_train, dl_aux
...
def validate_bounded_model(bounded_model: BoundedModel):
    ...
    for i, (pl, pn, pu) in enumerate(
        zip(bounded_model.param_l, bounded_model.param_n, bounded_model.param_u)
    ):
        ...
        if violations > 0 and max_violation > 1e-6:
            LOGGER.warning(
                "Nominal parameters not within certified bounds for parameter %s", i
            )
...
def log_run_start(
    ...
    LOGGER.debug(
        "\tLearning rate schedule: lr_decay=%s, lr_min=%s",
        config.lr_decay,
        config.lr_min,
    )
    ...
    LOGGER.debug(
        "\tGradient clipping: gamma=%s, method=%s",
        config.clip_gamma,
        config.clip_method,
    )
    ...
    if agt_type == "poison":
        LOGGER.debug(
            "\tAdversary feature-space budget: epsilon=%s, k_poison=%s",
            config.epsilon,
            config.k_poison,
        )
        LOGGER.debug(
    ...
    if config.paired_poison and config.label_k_poison != config.k_poison:
        LOGGER.info(
            "Using paired poisoning with different feature and label poisoning budgets."
        )
    if config.k_private > 0 and agt_type != "privacy":
        LOGGER.warning(
            "k_private is set to %s, but this is not a privacy run.", config.k_private
        )
    if config.k_unlearn > 0 and agt_type != "unlearning":
        LOGGER.warning(
            "k_unlearn is set to %s, but this is not an unlearning run.",
            config.k_unlearn,
        )
    if config.k_poison > 0 and agt_type != "poison":
        LOGGER.warning(
            "k_poison is set to %s, but this is not a poisoning run.", config.k_poison
        )
    if config.label_k_poison > 0 and agt_type != "poison":
        LOGGER.warning(
            "label_k_poison is set to %s, but this is not a poisoning run.",
            config.label_k_poison,
        )
    if config.clip_gamma == float("inf") and agt_type == "privacy":
        LOGGER.error(
            "Gradient clipping parameter must be set for privacy certified training."
        )
    if (
        max(config.k_unlearn, config.k_private, config.k_poison, config.label_k_poison)
        == 0
    ):
        LOGGER.info(
            "k=0 suffers from numerical instability in the bounds, consider using k > 0 or dtype double."
        )
