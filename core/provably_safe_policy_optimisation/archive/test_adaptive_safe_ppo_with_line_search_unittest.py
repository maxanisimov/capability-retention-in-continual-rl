...
class AdaptiveSafePPOTests(unittest.TestCase):
    ...
    def test_non_positive_line_search_probes_raises(self) -> None:
        with self.assertRaises(ValueError):
            self._make(line_search_probes=0)

    def test_default_strategy_is_line_search(self) -> None:
        model = self._make()
        diag = model.adaptive_diagnostics()
        self.assertEqual(diag["unsafe_update_strategy"], "line_search")
        self.assertEqual(diag["line_search_probes"], 32)
    ...
    def test_line_search_accepts_partial_step_without_rashomon(self) -> None:
        record: list = []
        self._patch_engine(record)  # would record if the engine were (wrongly) invoked
        model = self._make()
        self._force_unsafe_verifications(1)
        ...
        diag = model.adaptive_diagnostics()
        self.assertEqual(diag["candidates_verified_unsafe"], 1)
        self.assertEqual(diag["line_searches_run"], 1)
        self.assertEqual(diag["line_search_accepts"], 1)
        self.assertEqual(diag["line_search_probes_total"], 32)
        self.assertEqual(diag["rashomon_computations"], 0)
        self.assertEqual(diag["fallback_reverts"], 0)
        self.assertEqual(record, [])
        self.assertGreater(diag["last_line_search_alpha"], 0.0)
        self.assertLess(diag["last_line_search_alpha"], 1.0)
        self.assertGreaterEqual(model._greedy_safe_rate_now(), 1.0)
        # Probes do not count as candidate verifications: 2 minibatch candidates.
        self.assertEqual(diag["verifications_run"], 2)
        # Snapshot advanced to the accepted interpolated iterate.
        for param, snapshot in zip(model._live_actor_params, model._last_safe_params):
            self.assertTrue(th.equal(param.data, snapshot))

    def test_line_search_bisection_matches_analytic_boundary(self) -> None:
        probes = 12
        model = self._make(line_search_probes=probes)
        snap = [p.detach().clone() for p in model._live_actor_params]
        direction = [th.ones_like(p) for p in snap]
        with th.no_grad():
            for param, base, step in zip(model._live_actor_params, snap, direction):
                param.data.copy_(base + step)
        # Safe iff the L2 distance from iterate k stays within the ball whose
        # boundary the segment crosses at alpha = 0.3.
        total_norm = float(th.sqrt(sum((step**2).sum() for step in direction)))
        radius = 0.3 * total_norm

        def fake_rate() -> float:
            dist = float(
                th.sqrt(
                    sum(
                        ((param.data - base) ** 2).sum()
                        for param, base in zip(model._live_actor_params, snap)
                    )
                )
            )
            return 1.0 if dist <= radius else 0.0

        model._greedy_safe_rate_now = fake_rate
        model._line_search_to_safe()

        diag = model.adaptive_diagnostics()
        self.assertEqual(diag["line_searches_run"], 1)
        self.assertEqual(diag["line_search_accepts"], 1)
        self.assertEqual(diag["line_search_probes_total"], probes)
        alpha = diag["last_line_search_alpha"]
        self.assertAlmostEqual(alpha, 0.3, delta=2.0**-probes)
        # Final params lie on the segment at the accepted alpha; snapshot advanced.
        for param, base, step, snapshot in zip(
            model._live_actor_params, snap, direction, model._last_safe_params
        ):
            self.assertTrue(th.allclose(param.data, base + alpha * step, atol=1e-6))
            self.assertTrue(th.equal(param.data, snapshot))

    def test_line_search_reverts_when_no_step_is_safe(self) -> None:
        model = self._make()
        base = [p.detach().clone() for p in model._live_actor_params]
        with th.no_grad():
            for param in model._live_actor_params:
                param.data.add_(1.0)
        model._greedy_safe_rate_now = lambda: 0.0
        model._line_search_to_safe()

        diag = model.adaptive_diagnostics()
        self.assertEqual(diag["line_searches_run"], 1)
        self.assertEqual(diag["line_search_accepts"], 0)
        self.assertEqual(diag["fallback_reverts"], 1)
        self.assertEqual(diag["last_line_search_alpha"], 0.0)
        # Candidate fully reverted to iterate k; snapshot unchanged.
        for param, expected, snapshot in zip(
            model._live_actor_params, base, model._last_safe_params
        ):
            self.assertTrue(th.equal(param.data, expected))
            self.assertTrue(th.equal(snapshot, expected))
