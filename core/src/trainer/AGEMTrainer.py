...
class AGEMTrainer(SimpleTrainer):
    def __init__(
        self,
        model: nn.Module,
        paradigm: str = None,
        ...
        **kwargs: dict,
        ...
        super().__init__(model, paradigm, domain_map_fn, seed, **kwargs)
    ...
    def _train_step(
        ...
        **kwargs: dict,
        ...
        # 1. Sample a single batch from the memory buffer
        inputs_list, targets_list = zip(*memory_batches)
        mem_dataset = TensorDataset(torch.cat(inputs_list, dim=0), torch.cat(targets_list, dim=0))
        x_mem, y_mem = get_batch(mem_dataset, len(mem_dataset), shuffle=True, seed=self.seed, device=self.device)
        if hasattr(self, "domain_map_fn") and self.domain_map_fn:
            y_mem = self.domain_map_fn(y_mem)

        # 2. Compute a single gradient from the memory batch
        optimizer.zero_grad()
        ...
        # 3. Project the current gradient if it conflicts with the memory gradient
        dot_prod = grad_curr @ grad_mem
        if dot_prod < 0:
            # The quadprog solver is no longer needed for A-GEM's simple projection
            grad_tilde = grad_curr - (dot_prod / (grad_mem @ grad_mem)) * grad_mem
        else:
            grad_tilde = grad_curr  # No conflict, use the original gradient
