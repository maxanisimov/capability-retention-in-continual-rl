"""Train one scaled FrozenLake source layout across multiple seeds with CPU-core pinning."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train source PPO policy for one layout across multiple seeds, pinned one run per CPU core.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for per-seed launcher logs. Default: <output-dir>/<layout>/multi_seed_logs.",
    )
    parser.add_argument(
        "--poll-seconds",
        type=float,
...
def main() -> int:
    ...
    log_dir = args.log_dir or (args.output_dir / args.layout / "multi_seed_logs")
    ...
    print(
        f"Launching {len(seeds)} seed runs for layout={args.layout} with {len(core_pool)} available core(s): {core_pool}",
    )
    if len(core_pool) < len(seeds):
        print(
            "Note: fewer cores than seeds; runs will execute in waves while keeping one active run per core.",
        )
