"""Run EWC downstream adaptation across multiple seeds with CPU-core pinning."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run downstream_adaptation_ewc.py for one layout across multiple seeds, "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: <outputs-root>/<layout>/multi_seed_logs/downstream_ewc."
        ),
    )
    parser.add_argument(
        "--poll-seconds",
...
def main() -> int:
    ...
    log_dir = args.log_dir or (args.outputs_root / args.layout / "multi_seed_logs" / "downstream_ewc")
    ...
    print(
        f"Launching {len(seeds)} EWC downstream runs for layout={args.layout} "
    ...
    print("\nAll EWC downstream seed runs completed successfully.")
    return 0
