"""Run LunarLander Rashomon downstream adaptation across multiple seeds with CPU pinning."""
...
def _parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run downstream_adaptation_rashomon.py for one task setting across multiple seeds, "
    ...
    parser.add_argument(
        ...
        help="Subdirectory name used by downstream_adaptation_rashomon.py.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional override for number of source rollouts in Rashomon dataset creation.",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
        ...
        help=(
            ...
            "Default: <outputs-root>/<task-setting>/multi_seed_logs/downstream_rashomon."
        ),
    )
    parser.add_argument(
        "--poll-seconds",
...
def _build_worker_cmd(
    ...
    if args.rashomon_rollouts is not None:
        cmd.extend(["--rashomon-rollouts", str(args.rashomon_rollouts)])
...
def main() -> int:
    ...
    log_dir = args.log_dir or (
        args.outputs_root / args.task_setting / "multi_seed_logs" / "downstream_rashomon"
    )
    ...
    print(
        f"Launching {len(seeds)} downstream-Rashomon runs for task-setting={args.task_setting} "
    ...
    if passthrough:
        print(f"Forwarding extra args to downstream_adaptation_rashomon.py: {' '.join(passthrough)}")
    ...
    print("\nAll downstream Rashomon seed runs completed successfully.")
    return 0
