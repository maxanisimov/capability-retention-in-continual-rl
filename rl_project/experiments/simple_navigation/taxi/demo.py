"""
Taxi – Safe Continual Learning with SafeAdapt
==============================================

Demonstrates safe continual learning in the Gymnasium Taxi-v3 environment.
A taxi must navigate a 5x5 grid to pick up and deliver passengers at four
named locations (R, G, Y, B).  The challenge is adapting to a distribution
shift (Task 2 — different pickup/destination routes) while **certifiably**
maintaining safety on the original task (Task 1).

Safety here means avoiding illegal pickup/dropoff actions (the ones that
incur a -10 penalty in Taxi-v3).
...
Route constraints and training hyper-parameters are loaded from
``demo_configs.yaml``.
...
# --- Which route / hyper-parameter set from demo_configs.yaml ---------------
cfg_name: str = "top_to_bottom"
...
# Unpack YAML fields
task1_passenger_locs: list[int] = cfg["task1_passenger_locs"]
task1_dest_locs: list[int] = cfg["task1_dest_locs"]
task2_passenger_locs: list[int] = cfg["task2_passenger_locs"]
task2_dest_locs: list[int] = cfg["task2_dest_locs"]
seed: int = cfg["seed"]
...
num_eval_episodes: int = cfg.get("num_eval_episodes", 100)

# ── Seed everything ────────────────────────────────────────────────────────
set_all_seeds(seed)

# ── Print summary ──────────────────────────────────────────────────────────
_loc_str = lambda locs: ", ".join(LOC_NAMES[l] for l in locs)
print("=" * 72)
print("TAXI – SAFE CONTINUAL LEARNING EXPERIMENT")
print("=" * 72)
print(f"  Config           : {cfg_name}")
print(f"  Task 1 routes    : pickup [{_loc_str(task1_passenger_locs)}]"
      f" -> deliver [{_loc_str(task1_dest_locs)}]")
print(f"  Task 2 routes    : pickup [{_loc_str(task2_passenger_locs)}]"
      f" -> deliver [{_loc_str(task2_dest_locs)}]")
...
env1 = make_taxi_env(
    task_num=0,
    passenger_locs=task1_passenger_locs,
    dest_locs=task1_dest_locs,
)
env2 = make_taxi_env(
    task_num=1,
    passenger_locs=task2_passenger_locs,
    dest_locs=task2_dest_locs,
)

# rgb_array versions for plotting
env1_show = make_taxi_env(
    task_num=0,
    passenger_locs=task1_passenger_locs,
    dest_locs=task1_dest_locs,
    render_mode="rgb_array",
)
env2_show = make_taxi_env(
    task_num=1,
    passenger_locs=task2_passenger_locs,
    dest_locs=task2_dest_locs,
    render_mode="rgb_array",
...
print(f"  Task 1: pickup [{_loc_str(task1_passenger_locs)}]"
      f" -> deliver [{_loc_str(task1_dest_locs)}]")
print(f"  Task 2: pickup [{_loc_str(task2_passenger_locs)}]"
      f" -> deliver [{_loc_str(task2_dest_locs)}]")
...
multi_label = safe_state_action_data_name == "Sufficient Safety Data"
...
if train_unsafe_adapt and amnesic_actor is not None:
    results["UnsafeAdapt / Task 1"] = evaluate_policy(
        env1, amnesic_actor, num_eval_episodes,
    )
    results["UnsafeAdapt / Task 2"] = evaluate_policy(
        env2, amnesic_actor, num_eval_episodes,
    )
...
print(
    f"\nSafeAdapt perfect safety on Task 1:"
    f" {safe_t1 >= min_acc_limit}  (safety={safe_t1:.3f})"
)

# ── Save ───────────────────────────────────────────────────────────────────
if save_results:
    csv_name = (
        f"results_{cfg_name}_{safe_state_action_data_name.replace(' ', '_')}.csv"
    )
