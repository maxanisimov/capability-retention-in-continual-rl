...
def main() -> int:
    ...
    try:
        while pending or running:
            ...
            for job in running:
                ...
                print(f"[done] seed={job.seed} status={status} log={job.log_file}")
    ...
    print("\nSummary:")
    any_failure = False
    for seed, code, log_file in sorted(results, key=lambda x: x[0]):
        status = "ok" if code == 0 else f"fail({code})"
        print(f"  seed={seed:<4} status={status:<10} log={log_file}")
        if code != 0:
            any_failure = True
