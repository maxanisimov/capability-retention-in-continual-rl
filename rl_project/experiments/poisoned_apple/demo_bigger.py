...
env = PoisonedAppleEnv(
    ...
    # safe_apple_positions=[(1, 1), (2, 2), (4, 4)],
...
##########
### Train using ppo
# NOTE: if the only change is grid_size=10 (not 5), here is what I observe
#       total_timesteps=2_000 -> optimal policy for Env1; stalled in a start position in Env2
#       total_timesteps>=3_000 -> optimal policy for Env1; unsafe in Env2
ppo_cfg = PPOConfig(
    total_timesteps=3_000, # NOTE: for bigger grid, there needs to be more timesteps
)
...
env2 = PoisonedAppleEnv(
    ...
    # safe_apple_positions=[(4, 4)],
    poisoned_apple_positions=[(1, 1), (3, 3)],
    # poisoned_apple_positions=[(1, 1), (2, 2), (3, 3)],
    observation_type="flat",
    render_mode="human",
    max_steps=max_steps,
    seed=42
