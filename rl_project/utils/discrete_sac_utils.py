...
def discrete_sac_train(
    env: gym.Env, cfg: DiscreteSACConfig,
    actor_warm_start: nn.Sequential | None = None,
    critic_warm_start: nn.Sequential | None = None,
    ...
    Args:
        env: Gymnasium environment (must have discrete action space)
    ...
    assert isinstance(env.action_space, gym.spaces.Discrete), "Discrete SAC requires discrete action space"
    ...
    while global_step < cfg.total_timesteps:
        ...
        if return_training_data:
            ...
            training_data['actions'].append(action)  # type: ignore
        ...
        if global_step > cfg.learning_starts:
            ...
            # Update Q-networks
            with torch.no_grad():
                # Get next action probabilities from current policy
                next_logits = actor(next_states)
                next_probs = F.softmax(next_logits, dim=-1)
                next_log_probs = F.log_softmax(next_logits, dim=-1)

                # Get Q-values for next states from target networks
                next_q1 = qf1_target(next_states)
                next_q2 = qf2_target(next_states)
                next_q = torch.min(next_q1, next_q2)

                # Compute expected Q-value: E[Q - alpha * log(pi)]
                # For discrete actions: sum over all actions weighted by probabilities
                next_v = (next_probs * (next_q - alpha * next_log_probs)).sum(dim=-1, keepdim=True)

                # Compute target Q-value for the taken action
                target_q = rewards + (1 - dones) * cfg.gamma * next_v

            # Current Q-values for all actions
            current_q1 = qf1(states)
            current_q2 = qf2(states)

            # Select Q-values for the actions that were taken
            current_q1_a = current_q1.gather(1, actions.unsqueeze(1))
            current_q2_a = current_q2.gather(1, actions.unsqueeze(1))
            ...
            if global_step % cfg.policy_frequency == 0:
                ...
                # Get Q-values from current Q-networks (no grad through Q-networks)
                with torch.no_grad():
                    q1_pi = qf1(states)
                    q2_pi = qf2(states)
                ...
                # Equivalent to minimizing: E[alpha * log(pi) - Q]
                # For discrete: sum over all actions weighted by probabilities
                ...
                # Update alpha
                if cfg.autotune_alpha:
                    # Recompute for alpha update (need gradients)
                    logits = actor(states)
                    log_probs = F.log_softmax(logits, dim=-1)
                    probs = F.softmax(logits, dim=-1)

                    # Expected entropy: E[-log(pi)]
        ...
        if global_step % 10000 == 0 and global_step > 0:
            ...
            recent_reward = np.mean(episode_rewards) if len(episode_rewards) > 0 else 0.0
            log_msg = f"Steps={global_step} | evalR={mean_r:.1f}±{std_r:.1f} | recentR={recent_reward:.1f} | alpha={alpha:.3f} | elapsed={elapsed:.1f}s"
