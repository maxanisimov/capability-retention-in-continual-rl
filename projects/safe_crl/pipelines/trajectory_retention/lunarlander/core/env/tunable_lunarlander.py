...
LUNARLANDER_V4_ENTRY_POINT = "projects.safe_crl.pipelines.trajectory_retention.lunarlander.core.env.tunable_lunarlander:TunableLunarLander"
...
class TunableLunarLander(LunarLander):
    ...
    def step(self, action):  # type: ignore[override]
        """Step environment and attach API-semantic `info['is_success']`."""
        ...
        try:
            ...
            step_info = dict(info)
            is_success = bool(
                terminated and (not truncated) and np.isclose(float(reward), 100.0),
            )
            step_info["is_success"] = is_success
