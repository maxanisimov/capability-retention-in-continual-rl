...
# 
# The notebook uses:
# - `projects/safe_crl/pipelines/trajectory_retention/lunarlander/cli/launch_multi_seed.py` (mode = `downstream_rashomon`)
...
def find_repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in [p, *p.parents]:
        if (candidate / "pyproject.toml").exists() and (candidate / "projects" / "safe_crl").exists():
            return candidate
    raise RuntimeError("Could not find repository root from current working directory.")

REPO_ROOT = find_repo_root()
PIPELINE_ROOT = REPO_ROOT / "projects/safe_crl/pipelines/trajectory_retention/lunarlander"
