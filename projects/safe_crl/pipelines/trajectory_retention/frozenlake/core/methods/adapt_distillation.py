"""Adapt source policy to downstream scaled FrozenLake with demonstration distillation."""
...
def _resolve_distillation_cfg(settings: dict, layout: str, path: Path) -> dict:
    cfg = settings.get(layout, settings.get("default", {}))
    if not isinstance(cfg, dict):
        raise ValueError(f"Expected dict distillation config for layout '{layout}' in {path}.")
    distill_cfg = cfg.get("distillation", cfg)
    if not isinstance(distill_cfg, dict):
        raise ValueError(f"Expected dict distillation section for layout '{layout}' in {path}.")
    return distill_cfg
...
def main() -> None:
    ...
    parser.add_argument("--activation", type=str, choices=["tanh", "relu"], default="relu")
    ...
    if not isinstance(adapt_cfg, dict) or not isinstance(adapt_cfg.get("ppo"), dict):
        raise ValueError(f"Layout '{args.layout}' in {args.adapt_settings_file} must define a ppo section.")
    adapt_ppo_cfg = adapt_cfg["ppo"]
    distill_cfg = _resolve_distillation_cfg(distillation_settings, args.layout, args.distillation_settings_file)
    ...
    source_env_for_dim = make_env_from_layout(source_map, max_episode_steps, task_num=source_task_num, shaped=False)
    ...
    source_actor, source_critic = build_actor_critic(obs_dim=obs_dim, hidden=hidden, activation=args.activation)
    ...
    demo_env = make_env_from_layout(source_map, max_episode_steps, task_num=source_task_num, shaped=False)
    try:
        source_demo_dataset, source_state_action_pairs = create_source_trajectory_rashomon_dataset(
    ...
    train_env = make_env_from_layout(downstream_map, max_episode_steps, task_num=downstream_task_num, shaped=train_shaped)
    ...
    source_eval_env = make_env_from_layout(source_map, max_episode_steps, task_num=source_task_num, shaped=False)
    ...
    source_pairs_path = downstream_run_dir / "source_demo_state_action_pairs.yaml"
    ...
    source_pairs_path.write_text(yaml.safe_dump(source_state_action_pairs, sort_keys=False), encoding="utf-8")
    ...
    try:
        plot_episode(
            ...
            title=f"Distilled Policy on Source Task: {args.layout}",
        )
    finally:
    ...
    try:
        plot_episode(
            ...
            title=f"Distilled Policy on Downstream Task: {args.layout}",
        )
    finally:
    ...
    artifacts = {
        ...
        "source_demo_state_action_pairs_path": str(source_pairs_path),
    ...
    print(
        ...
        f"failure_rate={source_failure_rate:.3f}, demo_acc={demo_metrics['source_demo_accuracy']:.3f}",
    )
    print(
