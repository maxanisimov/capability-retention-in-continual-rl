"""Run the FrozenLake pipeline (source + selected methods) across seeds, CPU-pinned.

Each seed runs as one `run_seed_pipeline.py` subprocess pinned to one CPU core via the
shared `projects.safe_crl.pipelines._shared.multi_seed_launcher` scheduler, instead of
reimplementing job scheduling here. Replaces the old core/orchestration/launch_multi_seed.py,
launch_full_pipeline_multi_seed.py, and launch_downstream_all_multi_seed.py.
"""
...
METHOD_CHOICES = ("unconstrained", "ewc", "distillation", "rashomon")
...
def main(argv: list[str] | None = None) -> int:
    ...
    log_dir = args.log_dir or (seed_run_dir(args.outputs_root, args.layout, 0, rl=args.rl).parent / "multi_seed_logs")
