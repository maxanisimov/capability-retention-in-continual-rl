...
from projects.safe_crl.pipelines.safety_retention.FrozenLake.core.config import OBS_DIM, get_pipeline_config
from projects.safe_crl.pipelines.safety_retention.FrozenLake.core.models import build_actor_critic
from projects.safe_crl.pipelines.safety_retention.FrozenLake.core.paths import NOADAPT_POLICY_SUBDIR, mode_run_dir
...
from projects.safe_crl.utils.ppo_utils import ppo_train
...
def train_source(args: argparse.Namespace) -> Path:
    ...
    assert_source_policy_success(cfg, actor, seed=args.seed, device="cpu", label="Pre-finetune")
    ...
    source_rollout_env.close()
    assert_source_policy_success(cfg, actor, seed=args.seed, device="cpu", label="Post-finetune")
