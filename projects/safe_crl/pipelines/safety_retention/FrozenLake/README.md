...
- PPO, EWC, and Rashomon training settings mirror the existing FrozenLake
  `diagonal_10x10` settings from `projects/safe_crl/pipelines/trajectory_retention/frozenlake/settings`.
...
```bash
python projects/safe_crl/pipelines/safety_retention/frozenlake/run_pipeline.py \
...
```bash
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/train_source.py --pipeline diagonal_4x4 --seed 0
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/adapt_unconstrained.py --pipeline diagonal_4x4 --seed 0
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/adapt_ewc.py --pipeline diagonal_4x4 --seed 0
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/adapt_rashomon.py --pipeline diagonal_4x4 --seed 0
```

## Running one seed's full pipeline

```bash
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/run_seed_pipeline.py \
...
```bash
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/launch_multi_seed.py \
...
`projects/safe_crl/pipelines/_shared/multi_seed_launcher.py` scheduler (CPU-pinned,
one subprocess per seed, one log file per seed).

## Aggregating results

```bash
python projects/safe_crl/pipelines/safety_retention/frozenlake/cli/aggregate_metrics.py --pipeline diagonal_4x4
```
