"""MediaStreaming renderer ported from MASA-Safe-RL."""
...
class MediaStreamingRenderer:
    ...
    def _snapshot(self) -> "_MediaStreamingSnapshot":
        if hasattr(self.env, "_buffer_level_for_render"):
            state = int(self.env._buffer_level_for_render())
        else:
            state = int(self.env._start_state if self.env._state is None else self.env._state)
        start_state = int(getattr(self.env, "_start_buffer", self.env._start_state))
        return _MediaStreamingSnapshot(
            state=state,
            start_state=start_state,
            buffer_size=int(self.env._buffer_size),
            slow_rate=float(getattr(self.env, "_slow_rate", 0.0)),
            fast_rate=float(getattr(self.env, "_fast_rate", 0.0)),
            out_rate=float(getattr(self.env, "_out_rate", 0.0)),
