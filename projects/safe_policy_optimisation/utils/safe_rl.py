...
This module is environment-agnostic (it is used by every environment, not just
MiniPacman). It hosts the safe-RL baseline factories, evaluation/aggregation
helpers, checkpoint and GIF utilities, and the canonical result-IO helpers that
are re-exported from :mod:`projects.safe_policy_optimisation.utils.io`.

The matplotlib config directory is configured once on package import (see
``projects/safe_policy_optimisation/__init__.py``), so this module no longer
sets ``MPLCONFIGDIR`` itself.
"""
