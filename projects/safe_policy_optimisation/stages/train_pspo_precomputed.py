...
class ExecutedActionSafetyCounterWrapper(gym.Wrapper):
    ...
    def _state_from_obs(self, obs: Any) -> int:
        return obs_state_id(self.env, obs)
...
def evaluate_success_rate(
    ...
    *,
    ...
    shield = (
        Shield(shield_mask, obs_to_state=env.unwrapped.make_obs_to_state(), seed=seed)
        if eval_policy == "shielded"
        else None
    )
    rows: list[dict[str, Any]] = []
    try:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        # The base policy (and hence the PSPO actor) is built on whatever the
        # observation space flattens to: n one-hot dims in index mode, or the
        # feature-vector width in features mode.
    ...
    try:
        if args.evaluation_policy == "shielded":
            eval_shield = Shield(
                mask, obs_to_state=eval_env.unwrapped.make_obs_to_state(), seed=args.seed
            )
